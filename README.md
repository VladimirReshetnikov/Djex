...
Read the [rank-N and impredicative synthesis rules](docs/rank-n.md) for the supported language and search contract.
...
- For the next improvements and their acceptance requirements, see the
  [current synthesis re-triage](docs/reports/2026-09-14-synthesis-next-priorities.md).
...
native tree transfer checks and Haskell frontend/Church work follow. Provider filtering and
...
The [terminal integration](docs/reports/2026-09-08-combined-native-integration.md)
passes all ten gates and its final input-integrity check; Leant now pins the
tested `bfc3692e` dependency. Earlier deadline failures remain in the
...
Church composition. Native integration of this revision is complete; native
tree behavior requires separate acceptance.
...
kernel replays. That integration used canonical revision `c1ad560e`; the newer constructor release is recorded above.
The [constructor release](docs/reports/2026-09-13-contextual-constructor-use-acceptance.md)
subsequently closes the method/tail and nested-context failures with a complete
21-positive/six-control matrix and 711 passing native unit tests. Its source
selection, universe metadata and bounded search rules retain independent
checking. Remaining named-`where` entrances and broader binder/provider forms
stay on the [delivery plan](docs/reports/2026-09-13-synthesis-delivery-retriage.md).
...
`parseLeanLengthWhereSource` entrance admits `List.length`; Leant now wires it
to `:synth --where List.length result = List.length arg0 -- TYPE` with
conservative checked-target defaults. See the
[REPL guide](docs/repl.md#behavioral-constraints)
...
corpora with independent kernel replay. The later [fresh native integration](docs/reports/2026-09-09-one-shot-native-integration.md)
promotes Leant to **Djex `3adfac5c`**, with a strict build, **705 unit tests**
and **all ten fresh native gates** passing. Remaining public forms, native tree
synthesis and the full Church behavior target remain open.
