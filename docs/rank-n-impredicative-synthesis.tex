...
An already available exact opaque source for the whole goal keeps the
established forwarding route ahead of this speculative construction. For a
monomorphic goal, the deferred phase also tries exact forwarding first.
...
It is a guarantee about finding checked inhabitants under the configured
search policy, not an exhaustive enumeration of every typable program.
...
deduplication. Otherwise a request could traverse an unbounded sequence of
...
remaining lazy proposal stream. Resource accounting therefore bounds the work
used to discover candidates as well as the number eventually retained.
...
Both runs use a one-candidate window, 4,096 search steps, and a thirty-second
per-query timeout. They use Leant implementation revision \code{4757569}, with
...
The full ordinary transcript replay on that same executable remains pending.
That additional compatibility check is separate from the completed 700-case
Lean corpus and 90 compact fixture checks above.
