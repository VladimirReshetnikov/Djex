...
| `Language.Haskell.Synthesis.Semantic.Length.SMTLib` | Bounded canonical QF_LIA translation plus exact input-symbol, natural-input, origin, finite-box, and current guarded recursive piecewise-affine applicable-domain replay for nominally distinct checked scalar and binary-product Length problems; it projects each problem's unchanged candidate-independent bank scope, while the short applicable-domain wrappers only validate and associate evidence without launching or trusting a solver. |
