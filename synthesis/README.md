...
| `Language.Haskell.Synthesis.Semantic.Length.CounterexampleBank` | Candidate-independent nominal scalar/product scopes over the complete session/provider-law basis, solver-neutral policy, exact contract, and normalized target, plus strict bounded newest-first replay-input banks with input-only deduplication, deterministic tail eviction, explicit attempt caps, and exact statistics; a scope match authorizes only fresh replay and carries no verdict, receipt, proof, or pruning authority. |
...
| `Language.Haskell.Synthesis.Semantic.Length.Problem` | Atomic checked sessions and nominally distinct scalar/product typed-candidate behavioral problems: session-owned provider and restricted resolver authority, contract resealing, residual rejection, rigid root/provider authorization, mixed-role opaque targets, exact scalar zero/step cases inside product fields, provider-only consumption of independently authorized certificate carriers, and one retained candidate-independent counterexample-bank scope. |
| `Language.Haskell.Synthesis.Semantic.Length.SMTLib` | Bounded canonical QF_LIA translation plus exact input-symbol, natural-input, origin, finite-box, and current guarded recursive piecewise-affine applicable-domain replay for nominally distinct checked scalar and binary-product Length problems; it projects each problem's unchanged candidate-independent bank scope and exposes explicit same-scope operations that freshly reproduce a receipt before bounded insertion or replay one exact retained sample to mint current-query evidence, without automatically replaying any other sample, launching a solver, or trusting solver output. |
...
| `Language.Haskell.Synthesis.Semantic.Length.Where` | Bounded ASCII where-clause surface syntax for one Length postcondition: three byte- and nesting-bounded parser entrances over one grammar core -- compact `len(...)`, Haskell-shaped `length`/`fst`/`snd`, and Lean-shaped `List.length`/`.1`/`.2` spellings -- each producing the same opaque, source-free `LengthWhereSource`, and a separate elaborator that requires the caller's explicit scalar/binary-product domain and complete source-ordered role vector before lowering observed physical `argN` references onto the existing passive contract-source vocabulary; no spine, role, provider, solver, or execution authority is inferred. |
