...
positiveAffineApplicableDomainValidationTests :: TestTree
positiveAffineApplicableDomainValidationTests = testGroup
  "positive-affine applicable-domain validation"
  [ testCase
      "preserve literal v1 while publishing distinct scalar receipts" $ do
      Evaluate.lengthPositiveAffineApplicableDomainValidationSchemaTag @?=
        asciiBytes
          "finite-list-spine-length/positive-affine-precondition-domain-establishment/v1"
      assertBool "positive-affine validation reused the literal-v1 schema" $
        Evaluate.lengthPositiveAffineApplicableDomainValidationSchemaTag /=
          Evaluate.lengthApplicableDomainValidationSchemaTag
      let input = Length.LengthVariable $ Length.LengthInput 0
          result = Length.LengthVariable Length.LengthResult
          literalSource = contractWith
            (Length.LengthAtMost input $ Length.LengthLiteral 2)
            $ Length.LengthEqual result $ Length.LengthLiteral 7
      literalProblem <- adversarialConstantProviderProblem 7 literalSource
      literalV1 <- expectValidatedLengthApplicableDomain literalProblem
        $ Evaluate.validateLengthProblemApplicableDomain
            Evaluate.defaultLengthEvaluationLimits
            Evaluate.defaultLengthInputBoxLimits literalProblem
      let literalValidation =
            Evaluate.validateLengthProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits literalProblem
      literalAffine <- expectValidatedLengthPositiveAffineApplicableDomain
        literalProblem literalValidation
      providerName <- expectName "Fixture.problemReplayConstant"
      let providerBasis =
            Evaluate.FiniteSpineModelUnderAssumedProviderLaws [providerName]
      assertValidatedLengthApplicableDomain literalV1 [2] 3 3 providerBasis
      assertValidatedLengthPositiveAffineApplicableDomain
        literalAffine [2] 3 3 providerBasis
      force literalAffine `seq` pure ()

      literalEvidence <- case literalValidation of
        Right (Evaluate.LengthApplicableDomainEstablished established) ->
          pure established
        _ -> assertFailure
          "the literal affine domain did not retain positive evidence"
            >> error "unreachable"
      stale <- adversarialConstantProviderProblem 7
        $ contractWith
            (Length.LengthAtMost input $ Length.LengthLiteral 1)
      ...
      literalQuery <- expectRight $ SMTLib.sealLengthSMTLibQuery
        SMTLib.defaultLengthSMTLibLimits literalProblem
      literalQueryValidation <- expectRight
        $ SMTLib.validateLengthSMTLibQueryPositiveAffineApplicableDomain
            Evaluate.defaultLengthEvaluationLimits
            Evaluate.defaultLengthInputBoxLimits literalQuery
      case literalQueryValidation of
        Evaluate.LengthApplicableDomainEstablished queryReceipt ->
          queryReceipt @?= literalAffine
        Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
          ("the literal affine query was inapplicable: " ++ show reason)
        Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
          "the literal affine query returned a counterexample"

      let equalitySource = contractWith
            (Length.LengthEqual input $ Length.LengthLiteral 2)
            $ Length.LengthEqual result $ Length.LengthLiteral 7
      equalityProblem <- adversarialConstantProviderProblem 7 equalitySource
      v1Reason <- expectLengthApplicableDomainInapplicability
        $ Evaluate.validateLengthProblemApplicableDomain
            Evaluate.defaultLengthEvaluationLimits
            Evaluate.defaultLengthInputBoxLimits equalityProblem
      v1Reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
      equalityAffine <-
        expectValidatedLengthPositiveAffineApplicableDomain equalityProblem
          $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits equalityProblem
      assertValidatedLengthPositiveAffineApplicableDomain
        equalityAffine [2] 3 1 providerBasis
  , testCase
      "derive scaled additive maxima under either equality orientation" $ do
      let first = Length.LengthVariable $ Length.LengthInput 0
          second = Length.LengthVariable $ Length.LengthInput 1
          expression = Length.LengthSum
            [ Length.LengthScale 2 first
            , Length.LengthScale 3 second
            , Length.LengthLiteral 1
            ]
          literal = Length.LengthLiteral 11
          source precondition = contractWith precondition
            $ Length.LengthTruth True
          validate problem =
            Evaluate.validateLengthProblemPositiveAffineApplicableDomain
      ...
      mapM_ (\precondition -> do
          problem <- adversarialBinaryConstantZeroProblem
            $ source precondition
          receipt <- expectValidatedLengthPositiveAffineApplicableDomain problem
            $ validate problem
          assertValidatedLengthPositiveAffineApplicableDomain receipt
            [5, 3] 24 2 Evaluate.ProviderIndependentFiniteSpineModel
          force receipt `seq` pure ())
        [ Length.LengthEqual expression literal
        , Length.LengthEqual literal expression
        ]

      inequality <- adversarialBinaryConstantZeroProblem $ source
        $ Length.LengthAtMost expression $ Length.LengthLiteral 10
      inequalityReceipt <-
        expectValidatedLengthPositiveAffineApplicableDomain inequality
          $ validate inequality
      assertValidatedLengthPositiveAffineApplicableDomain inequalityReceipt
        [4, 3] 20 12 Evaluate.ProviderIndependentFiniteSpineModel

      let unary = Length.LengthVariable $ Length.LengthInput 0
      saturated <- adversarialConstantZeroProblem $ source
        $ Length.LengthAtMost
            (Length.LengthScale 1000000 unary)
            (Length.LengthLiteral 3)
      saturatedReceipt <-
        expectValidatedLengthPositiveAffineApplicableDomain saturated
          $ validate saturated
      assertValidatedLengthPositiveAffineApplicableDomain saturatedReceipt
        [0] 1 1 Evaluate.ProviderIndependentFiniteSpineModel

      saturatedConstant <- adversarialConstantZeroProblem $ source
        $ Length.LengthAtMost
            (Length.LengthSum
              [Length.LengthLiteral 1000000, unary])
            (Length.LengthLiteral 3)
      saturatedConstantReceipt <-
        expectValidatedLengthPositiveAffineApplicableDomain saturatedConstant
          $ validate saturatedConstant
      assertValidatedLengthPositiveAffineApplicableDomain
        saturatedConstantReceipt [0] 1 0
          Evaluate.ProviderIndependentFiniteSpineModel

      let repeated = Length.LengthSum
            [unary, Length.LengthScale 2 unary, Length.LengthLiteral 1]
          repeatedSource = contractWith
            (Length.LengthAll
              [ Length.LengthAtMost repeated $ Length.LengthLiteral 10
              , Length.LengthAtMost unary $ Length.LengthLiteral 2
              ])
            $ Length.LengthTruth True
      repeatedProblem <- adversarialConstantZeroProblem repeatedSource
      repeatedReceipt <-
        expectValidatedLengthPositiveAffineApplicableDomain repeatedProblem
          $ validate repeatedProblem
      assertValidatedLengthPositiveAffineApplicableDomain repeatedReceipt
        [2] 3 3 Evaluate.ProviderIndependentFiniteSpineModel

      unsupported <- adversarialConstantZeroProblem $ source
        $ Length.LengthAtMost
            (Length.LengthMinimum unary $ Length.LengthLiteral 1)
            (Length.LengthLiteral 2)
      unsupportedReason <-
        expectLengthPositiveAffineApplicableDomainInapplicability
          $ validate unsupported
      unsupportedReason @?=
        Evaluate.LengthApplicableDomainInputUpperBoundMissing 0

      legacyEquality <- adversarialBinaryConstantZeroProblem $ source
        $ Length.LengthEqual expression literal
      legacyReason <- expectLengthApplicableDomainInapplicability
        $ Evaluate.validateLengthProblemApplicableDomain
            Evaluate.defaultLengthEvaluationLimits
            Evaluate.defaultLengthInputBoxLimits legacyEquality
      legacyReason @?=
        Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
  , testCase
      "turn contradictions into one admitted all-zero box" $ do
      let source precondition = contractWith precondition
            $ Length.LengthTruth True
          validate problem =
            Evaluate.validateLengthProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits problem
          one = Length.LengthLiteral 1
          two = Length.LengthLiteral 2
          contradictions =
            [ Length.LengthTruth False
            , Length.LengthAtMost two one
            , Length.LengthEqual one two
            , Length.LengthEqual two one
            , Length.LengthAtMost
                (Length.LengthSum
                  [ Length.LengthVariable $ Length.LengthInput 0
                  , Length.LengthLiteral 2
      ...
      mapM_ (\precondition -> do
          ...
          receipt <- expectValidatedLengthPositiveAffineApplicableDomain problem
            $ validate problem
          assertValidatedLengthPositiveAffineApplicableDomain receipt
            [0] 1 0 Evaluate.ProviderIndependentFiniteSpineModel)
        contradictions

      let first = Length.LengthVariable $ Length.LengthInput 0
          lateContradiction = Length.LengthAll
            [ Length.LengthAtMost first $ Length.LengthLiteral 2
            , Length.LengthAtMost
                (Length.LengthSum [first, Length.LengthLiteral 2])
                $ Length.LengthLiteral 1
            ]
      lateProblem <- adversarialBinaryConstantZeroProblem
        $ source lateContradiction
      lateReceipt <- expectValidatedLengthPositiveAffineApplicableDomain
        lateProblem $ validate lateProblem
      assertValidatedLengthPositiveAffineApplicableDomain lateReceipt
        [0, 0] 1 0 Evaluate.ProviderIndependentFiniteSpineModel

      equalConstants <- adversarialConstantZeroProblem $ source
        $ Length.LengthEqual one one
      equalReason <- expectLengthPositiveAffineApplicableDomainInapplicability
        $ validate equalConstants
      equalReason @?=
        Evaluate.LengthApplicableDomainInputUpperBoundMissing 0

      let input = Length.LengthVariable $ Length.LengthInput 0
      zeroScaled <- adversarialConstantZeroProblem $ source
        $ Length.LengthAtMost
            (Length.LengthScale 0 input)
            (Length.LengthLiteral 0)
      zeroScaledReason <-
        expectLengthPositiveAffineApplicableDomainInapplicability
          $ validate zeroScaled
      zeroScaledReason @?=
        Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
  , testCase "establish scalar and product nullary singleton domains" $ do
      let scalarResult = Length.LengthVariable Length.LengthResult
          scalarSource = contractWith (Length.LengthTruth True)
            $ Length.LengthEqual scalarResult $ Length.LengthLiteral 0
      scalar <- adversarialZeroInputProblem scalarSource
      scalarReceipt <- expectValidatedLengthPositiveAffineApplicableDomain
        scalar
        $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
            Evaluate.defaultLengthEvaluationLimits
            Evaluate.defaultLengthInputBoxLimits scalar
      assertValidatedLengthPositiveAffineApplicableDomain scalarReceipt
        [] 1 1 Evaluate.ProviderIndependentFiniteSpineModel

      falseScalar <- adversarialZeroInputProblem
        $ contractWith (Length.LengthTruth False) $ Length.LengthTruth True
      falseScalarReceipt <-
        expectValidatedLengthPositiveAffineApplicableDomain falseScalar
          $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits falseScalar
      assertValidatedLengthPositiveAffineApplicableDomain falseScalarReceipt
        [] 1 0 Evaluate.ProviderIndependentFiniteSpineModel
      scalarOverflow <- adversarialZeroInputProblem
        $ contractWith (Length.LengthTruth True)
        $ Length.LengthEqual scalarResult $ Length.LengthLiteral 2
      let expectedScalarOverflow =
            Evaluate.LengthApplicableDomainInputBoxValidationRejected
              $ Evaluate.LengthInputBoxAssignmentEvaluationRejected 0
              $ Evaluate.LengthEvaluationValueBitLimitExceeded
                  Evaluate.LengthIntermediateValue 1 2
      assertLeft expectedScalarOverflow
        $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
            (evaluationLimitsWith 1 1)
            Evaluate.defaultLengthInputBoxLimits scalarOverflow

      let pairSource = spinePairContractWith
            (Length.LengthTruth True)
            $ Length.LengthAll
              [ Length.LengthEqual
                  (pairResultVariable Length.LengthSpinePairFirst)
      ...
      pair <- adversarialNullaryZeroSpinePairProblem pairSource
      pairReceipt <-
        expectValidatedLengthSpinePairPositiveAffineApplicableDomain pair
          $ Evaluate.validateLengthSpinePairProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits pair
      assertValidatedLengthSpinePairPositiveAffineApplicableDomain pairReceipt
        [] 1 1 Evaluate.ProviderIndependentFiniteSpineModel
      falsePair <- adversarialNullaryZeroSpinePairProblem
        $ spinePairContractWith
            (Length.LengthTruth False) (Length.LengthTruth True)
      falsePairReceipt <-
        expectValidatedLengthSpinePairPositiveAffineApplicableDomain falsePair
          $ Evaluate.validateLengthSpinePairProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits falsePair
      assertValidatedLengthSpinePairPositiveAffineApplicableDomain
        falsePairReceipt [] 1 0 Evaluate.ProviderIndependentFiniteSpineModel
      pairOverflow <- adversarialNullaryZeroSpinePairProblem
        $ spinePairContractWith (Length.LengthTruth True)
        $ Length.LengthEqual
            (pairResultVariable Length.LengthSpinePairSecond)
            $ Length.LengthLiteral 2
      let expectedPairOverflow =
            Evaluate.LengthSpinePairApplicableDomainInputBoxValidationRejected
              $ Evaluate.LengthSpinePairInputBoxAssignmentEvaluationRejected 0
              $ Evaluate.LengthSpinePairEvaluationValueBitLimitExceeded
                  Evaluate.LengthSpinePairIntermediateValue 1 2
      assertLeft expectedPairOverflow
        $ Evaluate.validateLengthSpinePairProblemPositiveAffineApplicableDomain
            (evaluationLimitsWith 1 1)
            Evaluate.defaultLengthInputBoxLimits pairOverflow
      force scalarReceipt `seq` force falseScalarReceipt `seq`
        force pairReceipt `seq` force falsePairReceipt `seq` pure ()
  , testCase
      "preserve width, value, product, and evaluation failure precedence" $ do
      binaryContradiction <- adversarialBinaryConstantZeroProblem
        $ contractWith (Length.LengthTruth False) $ Length.LengthTruth True
      narrow <- inputBoxLimits 1 16
      widthResult <- evaluateWithin
        $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
            (error "width rejection demanded evaluation limits")
            narrow binaryContradiction
      assertLeft
        (Evaluate.LengthApplicableDomainInputBoxValidationRejected
          $ Evaluate.LengthInputBoxProblemInputLimitExceeded 1 2)
        widthResult
      ...
      let input = Length.LengthVariable $ Length.LengthInput 0
          ...
          bounded maximumValue = contractWith
            ...
            $ Length.LengthTruth True
      tooWideValue <- adversarialConstantZeroProblem $ bounded 8
      assertLeft
        (Evaluate.LengthApplicableDomainInputBoxValidationRejected
          $ Evaluate.LengthInputBoxMaximumValueRejected 0
          $ Evaluate.LengthEvaluationValueBitLimitExceeded
        ...
        $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
            (evaluationLimitsWith 2 8)
            Evaluate.defaultLengthInputBoxLimits tooWideValue

      let first = Length.LengthVariable $ Length.LengthInput 0
          second = Length.LengthVariable $ Length.LengthInput 1
          productSource = contractWith
            (Length.LengthAll
              [ Length.LengthAtMost
                  (Length.LengthScale 2 first)
                  $ Length.LengthLiteral 2
            ...
            $ Length.LengthEqual result $ Length.LengthLiteral 1
      productProblem <- adversarialBinaryConstantZeroProblem productSource
      threeAssignments <- inputBoxLimits 2 3
      assertLeft
        (Evaluate.LengthApplicableDomainInputBoxValidationRejected
          $ Evaluate.LengthInputBoxAssignmentLimitExceeded 3 4)
        $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
            (evaluationLimitsWith 1 0) threeAssignments productProblem

      let isZero = Length.LengthEqual input $ Length.LengthLiteral 0
          evaluationSource = contractWith
            (Length.LengthAtMost input $ Length.LengthLiteral 1)
            $ Length.LengthEqual result
            $ Length.LengthIf isZero
                (Length.LengthLiteral 2)
          ...
          overflow = Evaluate.LengthEvaluationValueBitLimitExceeded
            Evaluate.LengthIntermediateValue 1 2
      evaluationProblem <- adversarialConstantZeroProblem evaluationSource
      assertLeft
        (Evaluate.LengthApplicableDomainInputBoxValidationRejected
          $ Evaluate.LengthInputBoxAssignmentEvaluationRejected 0 overflow)
        $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
            (evaluationLimitsWith 1 1)
            Evaluate.defaultLengthInputBoxLimits evaluationProblem
  , testCase
      "return the scalar lexicographic counterexample through its exact query" $
      do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            result = Length.LengthVariable Length.LengthResult
            source = contractWith
              ...
              $ Length.LengthEqual result
              $ Length.LengthSum [first, second]
        problem <- adversarialBinaryConstantZeroProblem source
        direct <- expectLengthPositiveAffineApplicableDomainCounterexample
          problem
          $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits problem
        Evaluate.validatedLengthCounterexampleInputs direct @?= [0, 1]
        Evaluate.validatedLengthCounterexampleResult direct @?= 0
        Evaluate.validatedLengthCounterexampleBasis direct @?=
          Evaluate.ProviderIndependentFiniteSpineModel
        ...
        queryValidation <- expectRight
          $ SMTLib.validateLengthSMTLibQueryPositiveAffineApplicableDomain
              ...
              Evaluate.defaultLengthInputBoxLimits query
        case queryValidation of
          Evaluate.LengthApplicableDomainCounterexample receipt ->
            receipt @?= direct
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            ("the affine query domain was inapplicable: " ++ show reason)
          Evaluate.LengthApplicableDomainEstablished{} -> assertFailure
            "the violating affine query domain produced positive evidence"
        SMTLib.lengthSMTLibQueryFingerprint query @?= fingerprintBefore
        SMTLib.lengthSMTLibQueryCheckBytes query @?= checkBytesBefore
        SMTLib.lengthSMTLibQueryInputSymbols query @?= symbolsBefore
        SMTLib.lengthSMTLibQueryInputValueRequestBytes query @?=
          valueRequestBefore
  , testCase
      "establish nominal product evidence and preserve pair query identity" $
      do
        Evaluate.lengthSpinePairPositiveAffineApplicableDomainValidationSchemaTag
          @?= asciiBytes
            "finite-binary-product-spine-lengths/positive-affine-precondition-domain-establishment/v1"
        assertBool "positive-affine product validation reused scalar schema" $
          Evaluate.lengthSpinePairPositiveAffineApplicableDomainValidationSchemaTag
            /=
              Evaluate.lengthPositiveAffineApplicableDomainValidationSchemaTag
        assertBool "positive-affine product validation reused literal v1" $
          Evaluate.lengthSpinePairPositiveAffineApplicableDomainValidationSchemaTag
            /= Evaluate.lengthSpinePairApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthSpinePairInput 0
            affine = Length.LengthSum
              [Length.LengthScale 2 input, Length.LengthLiteral 1]
            precondition = Length.LengthAtMost affine
        ...
        (evidence, receipt) <- case validation of
          Evaluate.LengthApplicableDomainEstablished established -> do
            ...
            pure (established, replayed)
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            ("the affine product domain was inapplicable: " ++ show reason)
              >> error "unreachable"
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "the safe affine product domain produced a counterexample"
              >> error "unreachable"
        assertValidatedLengthSpinePairPositiveAffineApplicableDomain receipt
          [2] 3 3 Evaluate.ProviderIndependentFiniteSpineModel
        force receipt `seq` pure ()
        ...
        let forgedScalarEvidence = unsafeCoerce evidence
              :: SemanticProblem.BehavioralEvidence
                  Length.FiniteListSpineLengthV1
                  Evaluate.ValidatedLengthPositiveAffineApplicableDomain
        assertLeft SemanticProblem.ReplayDomainMismatch
          $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem scalar)
              forgedScalarEvidence
        ...
        let fingerprintBefore =
            ...
            valueRequestBefore =
              SMTLib.lengthSpinePairSMTLibQueryInputValueRequestBytes query
        queryValidation <- expectRight
          $ SMTLib.validateLengthSpinePairSMTLibQueryPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits query
        case queryValidation of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            ("the query-owned affine product domain was inapplicable: " ++
              show reason)
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "the query-owned safe affine product domain returned a counterexample"
        SMTLib.lengthSpinePairSMTLibQueryFingerprint query @?=
          fingerprintBefore
        SMTLib.lengthSpinePairSMTLibQueryCheckBytes query @?= checkBytesBefore
        SMTLib.lengthSpinePairSMTLibQueryInputSymbols query @?= symbolsBefore
        ...
        let violatingSource = spinePairContractWith precondition
              $ Length.LengthEqual
                  (pairResultVariable Length.LengthSpinePairSecond)
                  $ Length.LengthLiteral 1
        violating <- adversarialInputAndZeroSpinePairProblem violatingSource
        directCounterexample <-
          expectLengthSpinePairPositiveAffineApplicableDomainCounterexample
            violating
            $ Evaluate.validateLengthSpinePairProblemPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits violating
        Evaluate.validatedLengthSpinePairCounterexampleInputs
          directCounterexample @?= [0]
        Evaluate.validatedLengthSpinePairCounterexampleResult
          directCounterexample @?= Length.LengthSpinePair 0 0
        violatingQuery <- expectRight
          $ SMTLib.sealLengthSpinePairSMTLibQuery
              SMTLib.defaultLengthSMTLibLimits violating
        violatingQueryValidation <- expectRight
          $ SMTLib.validateLengthSpinePairSMTLibQueryPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits violatingQuery
        case violatingQueryValidation of
          Evaluate.LengthApplicableDomainCounterexample queryCounterexample ->
            queryCounterexample @?= directCounterexample
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            ("the violating affine product query was inapplicable: " ++
              show reason)
          Evaluate.LengthApplicableDomainEstablished{} -> assertFailure
            "the violating affine product query produced positive evidence"
  ]

relationalPositiveAffineApplicableDomainValidationTests :: TestTree
relationalPositiveAffineApplicableDomainValidationTests = testGroup
  "relational positive-affine applicable-domain validation"
  [ testCase
      "preserve literal and positive-affine behavior under a third nominal receipt" $
      do
        Evaluate.lengthRelationalPositiveAffineApplicableDomainValidationSchemaTag
          @?= asciiBytes
            "finite-list-spine-length/relational-positive-affine-precondition-domain-establishment/v1"
        assertBool "relational validation reused the positive-affine schema"
          $ Evaluate.lengthRelationalPositiveAffineApplicableDomainValidationSchemaTag
              /= Evaluate.lengthPositiveAffineApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthInput 0
            result = Length.LengthVariable Length.LengthResult
            literalSource = contractWith
              (Length.LengthAtMost input $ Length.LengthLiteral 2)
              $ Length.LengthEqual result $ Length.LengthLiteral 7
        literal <- adversarialConstantProviderProblem 7 literalSource
        direct <- expectValidatedLengthApplicableDomain literal
          $ Evaluate.validateLengthProblemApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits literal
        affine <- expectValidatedLengthPositiveAffineApplicableDomain literal
          $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits literal
        relational <-
          expectValidatedLengthRelationalPositiveAffineApplicableDomain literal
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits literal
        providerName <- expectName "Fixture.problemReplayConstant"
        let providerBasis =
        ...
        assertValidatedLengthApplicableDomain
          direct [2] 3 3 providerBasis
        assertValidatedLengthPositiveAffineApplicableDomain
          affine [2] 3 3 providerBasis
        assertValidatedLengthRelationalPositiveAffineApplicableDomain
          relational [2] 3 3 providerBasis
        force relational `seq` pure ()

        let cancellation = contractWith
              (Length.LengthAtMost
                (Length.LengthScale 2 input)
                $ Length.LengthSum [input, Length.LengthLiteral 1])
              $ Length.LengthTruth True
        cancellationProblem <- adversarialConstantZeroProblem cancellation
        directReason <- expectLengthApplicableDomainInapplicability
          $ Evaluate.validateLengthProblemApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits cancellationProblem
        affineReason <- expectLengthPositiveAffineApplicableDomainInapplicability
          $ Evaluate.validateLengthProblemPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits cancellationProblem
        directReason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
        affineReason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
        cancellationReceipt <-
          expectValidatedLengthRelationalPositiveAffineApplicableDomain
            cancellationProblem
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits cancellationProblem
        assertValidatedLengthRelationalPositiveAffineApplicableDomain
          cancellationReceipt [1] 2 2
            Evaluate.ProviderIndependentFiniteSpineModel

        let commonConstantCancellation = contractWith
              (Length.LengthAtMost
                (Length.LengthSum
                  [ Length.LengthScale 2 input
                  , Length.LengthLiteral 3
                ...
                $ Length.LengthSum [input, Length.LengthLiteral 4])
              $ Length.LengthTruth True
        commonConstantProblem <- adversarialConstantZeroProblem
          commonConstantCancellation
        commonConstantReceipt <-
          expectValidatedLengthRelationalPositiveAffineApplicableDomain
            commonConstantProblem
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits commonConstantProblem
        assertValidatedLengthRelationalPositiveAffineApplicableDomain
          commonConstantReceipt [1] 2 2
            Evaluate.ProviderIndependentFiniteSpineModel
  , testCase
      "propagate equality in either direction and retry multi-hop chains" $ do
      let first = Length.LengthVariable $ Length.LengthInput 0
          second = Length.LengthVariable $ Length.LengthInput 1
          doubledSecond = Length.LengthSum [second, second]
          ...
          validate problem =
            Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits problem
          equalityOrientations =
            [ Length.LengthEqual first doubledSecond
            , Length.LengthEqual doubledSecond first
            ]
          seeds =
            [ Length.LengthAtMost first $ Length.LengthLiteral 6
            , Length.LengthAtMost second $ Length.LengthLiteral 3
            ]
      mapM_ (\(equality, seed) -> do
          problem <- adversarialBinaryConstantZeroProblem
            $ source [equality, seed]
          receipt <-
            expectValidatedLengthRelationalPositiveAffineApplicableDomain
          ...
          assertValidatedLengthRelationalPositiveAffineApplicableDomain
            receipt [6, 3] 28 4
              Evaluate.ProviderIndependentFiniteSpineModel)
        [(equality, seed) | equality <- equalityOrientations, seed <- seeds]

      let third = Length.LengthVariable $ Length.LengthInput 2
          multiHop = source
            [ Length.LengthAtMost first $ Length.LengthScale 2 second
            , Length.LengthAtMost second
                $ Length.LengthSum [third, Length.LengthLiteral 1]
            , Length.LengthAtMost third $ Length.LengthLiteral 2
            ]
      multiHopProblem <-
        adversarialWideConstantZeroProblemWithContract 3 multiHop
      multiHopReceipt <-
        expectValidatedLengthRelationalPositiveAffineApplicableDomain
          multiHopProblem $ validate multiHopProblem
      assertValidatedLengthRelationalPositiveAffineApplicableDomain
        multiHopReceipt [6, 3, 2] 84 29
          Evaluate.ProviderIndependentFiniteSpineModel
  , testCase
      "fire each source-ordered rule once without seeking a least numeric fixed point" $
      do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            third = Length.LengthVariable $ Length.LengthInput 2
            precondition = Length.LengthAll
              [ Length.LengthAtMost first second
              , Length.LengthAtMost second $ Length.LengthLiteral 10
              , Length.LengthAtMost second third
              , Length.LengthAtMost third $ Length.LengthLiteral 2
              ]
            source = contractWith precondition $ Length.LengthTruth True
        problem <- adversarialWideConstantZeroProblemWithContract 3 source
        bounded <- evaluateWithin
          $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits problem
        receipt <-
          expectValidatedLengthRelationalPositiveAffineApplicableDomain
            problem bounded
        -- x <= y fires using y <= 10 before y is later tightened through z.
        -- Retiring that rule leaves the sound, deliberately nonleast x <= 10.
        assertValidatedLengthRelationalPositiveAffineApplicableDomain
          receipt [10, 2, 2] 99 10
            Evaluate.ProviderIndependentFiniteSpineModel
  , testCase
      "prioritize width, contradiction, values, product, and evaluation exactly" $
      do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            result = Length.LengthVariable Length.LengthResult
            contradictionSource = contractWith
              (Length.LengthAll
                [ Length.LengthAtMost
                    (Length.LengthSum [first, Length.LengthLiteral 2]) second
                , Length.LengthAtMost second $ Length.LengthLiteral 1
                ])
              $ Length.LengthTruth True
        contradiction <- adversarialBinaryConstantZeroProblem
          contradictionSource
        contradictionReceipt <-
          expectValidatedLengthRelationalPositiveAffineApplicableDomain
            ...
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits contradiction
        assertValidatedLengthRelationalPositiveAffineApplicableDomain
          contradictionReceipt [0, 0] 1 0
            Evaluate.ProviderIndependentFiniteSpineModel

        immediateContradiction <- adversarialBinaryConstantZeroProblem
          $ contractWith (Length.LengthTruth False) $ Length.LengthTruth True
        immediateReceipt <-
          expectValidatedLengthRelationalPositiveAffineApplicableDomain
            ...
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits immediateContradiction
        assertValidatedLengthRelationalPositiveAffineApplicableDomain
          immediateReceipt [0, 0] 1 0
            Evaluate.ProviderIndependentFiniteSpineModel
        ...
        let unary = Length.LengthVariable $ Length.LengthInput 0
            tooWideSource = contractWith
              (Length.LengthAtMost
                (Length.LengthScale 2 unary)
                $ Length.LengthSum [unary, Length.LengthLiteral 4])
              $ Length.LengthTruth True
        tooWide <- adversarialConstantZeroProblem tooWideSource
        assertLeft
          (Evaluate.LengthApplicableDomainInputBoxValidationRejected
            $ Evaluate.LengthInputBoxMaximumValueRejected 0
            $ Evaluate.LengthEvaluationValueBitLimitExceeded
          ...
          $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
              (evaluationLimitsWith 2 8)
              Evaluate.defaultLengthInputBoxLimits tooWide

        let productSource = contractWith
              (Length.LengthAll
                [ Length.LengthAtMost first $ Length.LengthLiteral 1
                , Length.LengthAtMost second $ Length.LengthLiteral 1
                ])
        ...
        let isZero = Length.LengthEqual unary $ Length.LengthLiteral 0
            evaluationSource = contractWith
              (Length.LengthAtMost unary $ Length.LengthLiteral 1)
              $ Length.LengthEqual result
              $ Length.LengthIf isZero
                  (Length.LengthLiteral 2)
                  (Length.LengthLiteral 1)
        ...
        unsupported <- adversarialConstantZeroProblem $ contractWith
          (Length.LengthAtMost
            (Length.LengthMinimum unary $ Length.LengthLiteral 1)
            $ Length.LengthLiteral 2)
          $ Length.LengthTruth True
        unsupportedReason <-
          expectLengthRelationalPositiveAffineApplicableDomainInapplicability
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits unsupported
        unsupportedReason @?=
          Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
        missing <- adversarialBinaryConstantZeroProblem $ contractWith
          (Length.LengthAtMost first second) $ Length.LengthTruth True
        missingReason <-
          expectLengthRelationalPositiveAffineApplicableDomainInapplicability
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits missing
        missingReason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
  , testCase
      "return the first scalar counterexample with provider and query identity" $
      do
        let input = Length.LengthVariable $ Length.LengthInput 0
            result = Length.LengthVariable Length.LengthResult
            bound = Length.LengthAtMost
              (Length.LengthScale 2 input)
              $ Length.LengthSum [input, Length.LengthLiteral 1]
            safeSource = contractWith bound
              $ Length.LengthEqual result $ Length.LengthLiteral 7
        safe <- adversarialConstantProviderProblem 7 safeSource
        let validation =
              Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits safe
        receipt <-
          expectValidatedLengthRelationalPositiveAffineApplicableDomain
            safe validation
        providerName <- expectName "Fixture.problemReplayConstant"
        let basis = Evaluate.FiniteSpineModelUnderAssumedProviderLaws
              [providerName]
        assertValidatedLengthRelationalPositiveAffineApplicableDomain
          receipt [1] 2 2 basis
        evidence <- case validation of
          Right (Evaluate.LengthApplicableDomainEstablished retained) ->
            pure retained
          Left failure -> assertFailure
            ("safe relational validation was rejected: " ++ show failure)
              >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainInapplicable reason) ->
            assertFailure
              ("safe relational validation was inapplicable: " ++ show reason)
                >> error "unreachable"
          Right Evaluate.LengthApplicableDomainCounterexample{} ->
            assertFailure "safe relational validation found a counterexample"
              >> error "unreachable"
        stale <- adversarialConstantProviderProblem 7
          $ contractWith
              (Length.LengthAtMost input $ Length.LengthLiteral 0)
              $ Length.LengthEqual result $ Length.LengthLiteral 7
        assertLeft SemanticProblem.ReplayEncodingFingerprintMismatch
          $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem stale)
        ...
        case queryValidation of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            $ "query-owned relational validation was inapplicable: "
                ++ show reason
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "query-owned safe relational validation found a counterexample"
        SMTLib.lengthSMTLibQueryFingerprint query @?= fingerprintBefore
        SMTLib.lengthSMTLibQueryCheckBytes query @?= checkBefore
        SMTLib.lengthSMTLibQueryInputSymbols query @?= symbolsBefore
        SMTLib.lengthSMTLibQueryInputValueRequestBytes query @?= valuesBefore

        violating <- adversarialConstantProviderProblem 7
          $ contractWith bound $ Length.LengthEqual result input
        counterexample <-
          expectLengthRelationalPositiveAffineApplicableDomainCounterexample
            violating
            $ Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits violating
        Evaluate.validatedLengthCounterexampleInputs counterexample @?= [0]
        Evaluate.validatedLengthCounterexampleResult counterexample @?= 7
        Evaluate.validatedLengthCounterexampleBasis counterexample @?= basis
  , testCase
      "establish nominal product evidence and preserve pair query identity" $
      do
        Evaluate.lengthSpinePairRelationalPositiveAffineApplicableDomainValidationSchemaTag
          @?= asciiBytes
            "finite-binary-product-spine-lengths/relational-positive-affine-precondition-domain-establishment/v1"
        assertBool "relational product validation reused scalar schema"
          $ Evaluate.lengthSpinePairRelationalPositiveAffineApplicableDomainValidationSchemaTag
              /= Evaluate.lengthRelationalPositiveAffineApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthSpinePairInput 0
            bound = Length.LengthAtMost
              (Length.LengthScale 2 input)
              $ Length.LengthSum [input, Length.LengthLiteral 1]
            safeSource = spinePairContractWith bound
              $ Length.LengthAll
                  [ Length.LengthEqual
                      (pairResultVariable Length.LengthSpinePairFirst) input
                  , Length.LengthEqual
        ...
        safe <- adversarialInputAndZeroSpinePairProblem safeSource
        receipt <-
          expectValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain
            safe
            $ Evaluate.validateLengthSpinePairProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits safe
        assertValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain
          receipt [1] 2 2 Evaluate.ProviderIndependentFiniteSpineModel
        force receipt `seq` pure ()
        query <- expectRight $ SMTLib.sealLengthSpinePairSMTLibQuery
          SMTLib.defaultLengthSMTLibLimits safe
        let fingerprintBefore =
            ...
            valuesBefore =
              SMTLib.lengthSpinePairSMTLibQueryInputValueRequestBytes query
        queryValidation <- expectRight
          $ SMTLib.validateLengthSpinePairSMTLibQueryRelationalPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits query
        case queryValidation of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            $ "query-owned relational product validation was inapplicable: "
                ++ show reason
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "query-owned safe relational product validation found a counterexample"
        SMTLib.lengthSpinePairSMTLibQueryFingerprint query @?=
          fingerprintBefore
        ...
        violating <- adversarialInputAndZeroSpinePairProblem
          $ spinePairContractWith bound
          $ Length.LengthEqual
              (pairResultVariable Length.LengthSpinePairSecond)
              $ Length.LengthLiteral 1
        counterexample <-
          expectLengthSpinePairRelationalPositiveAffineApplicableDomainCounterexample
            violating
            $ Evaluate.validateLengthSpinePairProblemRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits violating
        Evaluate.validatedLengthSpinePairCounterexampleInputs counterexample @?=
          [0]
        Evaluate.validatedLengthSpinePairCounterexampleResult counterexample @?=
          Length.LengthSpinePair 0 0
  ]

strictRelationalPositiveAffineApplicableDomainValidationTests :: TestTree
strictRelationalPositiveAffineApplicableDomainValidationTests = testGroup
  "strict relational positive-affine applicable-domain validation"
  [ testCase "rewrite strict scalar clauses with exact natural algebra" $ do
      let input = Length.LengthVariable $ Length.LengthInput 0
          ...
          validateLegacy problem =
            Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits problem
          assertStrict precondition maximums assignments applicable = do
            problem <- adversarialConstantZeroProblem $ source precondition
            legacyReason <-
              expectLengthRelationalPositiveAffineApplicableDomainInapplicability
                $ validateLegacy problem
            ...
            receipt <-
              expectValidatedLengthStrictRelationalPositiveAffineApplicableDomain
                problem $ validate problem
            assertValidatedLengthStrictRelationalPositiveAffineApplicableDomain
      ...
      assertStrict
        (Length.LengthNot $ Length.LengthAtMost (literal 5)
          $ Length.LengthSum
              [Length.LengthScale 2 input, literal 1])
        [1] 2 2
      assertStrict
        (Length.LengthNot $ Length.LengthAtMost
          (Length.LengthSum [input, literal 1])
          $ Length.LengthScale 2 input)
        [0] 1 1
      assertStrict
        (Length.LengthNot $ Length.LengthAtMost
          (Length.LengthScale 2 input)
          $ Length.LengthSum [input, input])
        [0] 1 0
  , testCase "close source-ordered strict chains and retain lower exclusions" $ do
      let first = Length.LengthVariable $ Length.LengthInput 0
          second = Length.LengthVariable $ Length.LengthInput 1
          third = Length.LengthVariable $ Length.LengthInput 2
          source clauses = contractWith (Length.LengthAll clauses)
          ...
          validateLegacy problem =
            Evaluate.validateLengthProblemRelationalPositiveAffineApplicableDomain
              ...
              Evaluate.defaultLengthInputBoxLimits problem
          chain = source
            [ Length.LengthNot $ Length.LengthAtMost second first
            , Length.LengthNot $ Length.LengthAtMost third second
            , Length.LengthAtMost third $ Length.LengthLiteral 5
            ]
      chainProblem <-
        adversarialWideConstantZeroProblemWithContract 3 chain
      chainLegacy <-
        expectLengthRelationalPositiveAffineApplicableDomainInapplicability
          $ validateLegacy chainProblem
      chainLegacy @?=
        Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
      chainReceipt <-
        expectValidatedLengthStrictRelationalPositiveAffineApplicableDomain
          chainProblem $ validate chainProblem
      assertValidatedLengthStrictRelationalPositiveAffineApplicableDomain
        chainReceipt [3, 4, 5] 120 20
          Evaluate.ProviderIndependentFiniteSpineModel

      let bounded maximumValue = source
            [ Length.LengthAtMost first
                $ Length.LengthLiteral maximumValue
            , Length.LengthNot $ Length.LengthAtMost first
                $ Length.LengthLiteral 5
            ]
      retained <- adversarialConstantZeroProblem $ bounded 10
      retainedReceipt <-
        expectValidatedLengthStrictRelationalPositiveAffineApplicableDomain
          retained $ validate retained
      assertValidatedLengthStrictRelationalPositiveAffineApplicableDomain
        retainedReceipt [10] 11 5
          Evaluate.ProviderIndependentFiniteSpineModel
      contradiction <- adversarialConstantZeroProblem $ bounded 4
      contradictionReceipt <-
        expectValidatedLengthStrictRelationalPositiveAffineApplicableDomain
          contradiction $ validate contradiction
      assertValidatedLengthStrictRelationalPositiveAffineApplicableDomain
        contradictionReceipt [0] 1 0
          Evaluate.ProviderIndependentFiniteSpineModel
  , testCase "ignore every unsupported negated whole clause" $ do
      let input = Length.LengthVariable $ Length.LengthInput 0
          literal value = Length.LengthLiteral value
          source precondition = contractWith precondition
            $ Length.LengthTruth True
          ignoredClauses =
            [ Length.LengthNot $ Length.LengthEqual input $ literal 2
            , Length.LengthNot $ Length.LengthAll
      ...
      let second = Length.LengthVariable $ Length.LengthInput 1
      missingFirst <- adversarialBinaryConstantZeroProblem $ source
        $ Length.LengthNot $ Length.LengthAtMost (literal 5) second
      missingReason <-
        expectLengthStrictRelationalPositiveAffineApplicableDomainInapplicability
          $ Evaluate.validateLengthProblemStrictRelationalPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits missingFirst
      missingReason @?=
        Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
  , testCase "preserve the legacy relational domain on every old clause" $ do
      let input = Length.LengthVariable $ Length.LengthInput 0
          sources =
            [ contractWith
                (Length.LengthAtMost input $ Length.LengthLiteral 3)
                $ Length.LengthTruth True
            , contractWith
                (Length.LengthAtMost
                  (Length.LengthScale 2 input)
                  $ Length.LengthSum
                      [input, Length.LengthLiteral 1])
                $ Length.LengthTruth True
            ]
      mapM_ (\source -> do
          ...
          Evaluate.validatedLengthStrictRelationalPositiveAffineApplicableDomainInclusiveMaximums
              strict @?=
            Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainInclusiveMaximums
              legacy
          Evaluate.validatedLengthStrictRelationalPositiveAffineApplicableDomainAssignmentCount
              strict @?=
            Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainAssignmentCount
              legacy
          Evaluate.validatedLengthStrictRelationalPositiveAffineApplicableDomainApplicableAssignmentCount
              strict @?=
            Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainApplicableAssignmentCount
              legacy
          Evaluate.validatedLengthStrictRelationalPositiveAffineApplicableDomainBasis
              strict @?=
            Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainBasis
              legacy)
        sources
  , testCase "retain strict validation precedence before exhaustive replay" $ do
      let first = Length.LengthVariable $ Length.LengthInput 0
          second = Length.LengthVariable $ Length.LengthInput 1
          result = Length.LengthVariable Length.LengthResult
          strictBound value variable = Length.LengthNot
            $ Length.LengthAtMost (Length.LengthLiteral value) variable
      ...
      binary <- adversarialBinaryConstantZeroProblem
        $ source (strictBound 5 second) $ Length.LengthTruth True
      narrow <- inputBoxLimits 1 16
      width <- evaluateWithin
      ...
      oversized <- adversarialConstantZeroProblem
        $ source (strictBound 8 first) $ Length.LengthTruth True
      assertLeft
        (Evaluate.LengthApplicableDomainInputBoxValidationRejected
          $ Evaluate.LengthInputBoxMaximumValueRejected 0
          $ Evaluate.LengthEvaluationValueBitLimitExceeded
              (Evaluate.LengthProblemInputValue 0) 2 3)
        $ validate (evaluationLimitsWith 2 8)
            Evaluate.defaultLengthInputBoxLimits oversized

      pairProblem <- adversarialBinaryConstantZeroProblem $ source
        (Length.LengthAll [strictBound 2 first, strictBound 2 second])
        $ Length.LengthEqual result $ Length.LengthLiteral 1
      threeAssignments <- inputBoxLimits 2 3
      assertLeft
        (Evaluate.LengthApplicableDomainInputBoxValidationRejected
          $ Evaluate.LengthInputBoxAssignmentLimitExceeded 3 4)
        $ validate (evaluationLimitsWith 1 0) threeAssignments pairProblem
  ...
  , testCase
      ...
      do
        ...
        assertBool "strict validation reused the legacy relational schema"
          $ Evaluate.lengthStrictRelationalPositiveAffineApplicableDomainValidationSchemaTag
              /= Evaluate.lengthRelationalPositiveAffineApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthInput 0
            precondition = Length.LengthNot $ Length.LengthAtMost
              (Length.LengthLiteral 5) $ Length.LengthScale 2 input
            source = contractWith precondition $ Length.LengthTruth True
        problem <- adversarialConstantZeroProblem source
        ...
        assertValidatedLengthStrictRelationalPositiveAffineApplicableDomain
          receipt [2] 3 3 Evaluate.ProviderIndependentFiniteSpineModel
        evidence <- case directValidation of
          Right (Evaluate.LengthApplicableDomainEstablished established) ->
            pure established
          _ -> assertFailure "strict scalar validation lost positive evidence"
            >> error "unreachable"
        stale <- adversarialConstantZeroProblem
          $ contractWith
              (Length.LengthNot $ Length.LengthAtMost
                (Length.LengthLiteral 3) input)
              $ Length.LengthTruth True
        assertLeft SemanticProblem.ReplayEncodingFingerprintMismatch
          $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem stale)
        ...
        case legacyQuery of
          Evaluate.LengthApplicableDomainInapplicable reason ->
            reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
          _ -> assertFailure "legacy query validation accepted a strict-only bound"
        queryValidation <- expectRight
        ...
        case queryValidation of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            $ "strict scalar query validation was inapplicable: " ++ show reason
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "strict scalar query validation found a counterexample"
        SMTLib.lengthSMTLibQueryFingerprint query @?= fingerprintBefore
        SMTLib.lengthSMTLibQueryCheckBytes query @?= checkBefore
        SMTLib.lengthSMTLibQueryInputSymbols query @?= symbolsBefore
        SMTLib.lengthSMTLibQueryInputValueRequestBytes query @?= valuesBefore
  , testCase
      "keep product evidence nominal and preserve pair query identity" $ do
        Evaluate.lengthSpinePairStrictRelationalPositiveAffineApplicableDomainValidationSchemaTag
          @?= asciiBytes
            "finite-binary-product-spine-lengths/strict-relational-positive-affine-precondition-domain-establishment/v1"
        assertBool "strict product validation reused the scalar schema"
          $ Evaluate.lengthSpinePairStrictRelationalPositiveAffineApplicableDomainValidationSchemaTag
              /= Evaluate.lengthStrictRelationalPositiveAffineApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthSpinePairInput 0
            precondition = Length.LengthNot $ Length.LengthAtMost
        ...
        problem <- adversarialInputAndZeroSpinePairProblem source
        let directValidation =
              Evaluate.validateLengthSpinePairProblemStrictRelationalPositiveAffineApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits problem
        receipt <-
          expectValidatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomain
            problem directValidation
        assertValidatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomain
          receipt [2] 3 3 Evaluate.ProviderIndependentFiniteSpineModel
        productEvidence <- case directValidation of
          Right (Evaluate.LengthApplicableDomainEstablished established) ->
            pure established
          _ -> assertFailure "strict product validation lost positive evidence"
            >> error "unreachable"
        ...
        let forgedScalarEvidence = unsafeCoerce productEvidence
              :: SemanticProblem.BehavioralEvidence
                  Length.FiniteListSpineLengthV1
                  Evaluate.ValidatedLengthStrictRelationalPositiveAffineApplicableDomain
        assertLeft SemanticProblem.ReplayDomainMismatch
          $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem scalar)
              forgedScalarEvidence
        ...
        let fingerprintBefore =
            ...
            valuesBefore =
              SMTLib.lengthSpinePairSMTLibQueryInputValueRequestBytes query
        queryValidation <- expectRight
          $ SMTLib.validateLengthSpinePairSMTLibQueryStrictRelationalPositiveAffineApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits query
        case queryValidation of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            $ "strict product query validation was inapplicable: " ++ show reason
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "strict product query validation found a counterexample"
        SMTLib.lengthSpinePairSMTLibQueryFingerprint query @?=
          fingerprintBefore
        SMTLib.lengthSpinePairSMTLibQueryCheckBytes query @?= checkBefore
        SMTLib.lengthSpinePairSMTLibQueryInputSymbols query @?= symbolsBefore
...
strictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationTests =
  testGroup
    "strict relational positive-affine quotient root-extrema applicable-domain validation"
    [ testCase "apply all four exact extrema laws and propagate their rules" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            third = Length.LengthVariable $ Length.LengthInput 2
            literal value = Length.LengthLiteral value
            source precondition = contractWith precondition
              $ Length.LengthTruth True
            validate problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits problem
            assertUnary precondition maximums assignments applicable = do
              problem <- adversarialConstantZeroProblem $ source precondition
              receipt <-
                expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                  problem $ validate problem
              assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                receipt maximums assignments applicable
                  Evaluate.ProviderIndependentFiniteSpineModel

        -- max(x, 2x + 1) <= 5 gives x <= 5 and 2x + 1 <= 5.
        assertUnary
          (Length.LengthAtMost
        ...
        -- 2x + 1 <= min(x + 5, 7) gives both directed constraints.
        assertUnary
          (Length.LengthAtMost
            (Length.LengthSum [Length.LengthScale 2 first, literal 1])
            $ Length.LengthMinimum
                (Length.LengthSum [first, literal 5]) (literal 7))
          [3] 4 4

        -- not (min(x + 4, 9) <= 2x) gives 2x + 1 <= x + 4 and <= 9.
        assertUnary
          (Length.LengthNot $ Length.LengthAtMost
            (Length.LengthMinimum
              (Length.LengthSum [first, literal 4]) (literal 9))
            $ Length.LengthScale 2 first)
          [3] 4 4

        -- not (5 <= max(2x, x + 1)) gives 2x + 1 <= 5 and x + 2 <= 5.
        assertUnary
          (Length.LengthNot $ Length.LengthAtMost (literal 5)
            $ Length.LengthMaximum
                (Length.LengthScale 2 first)
                (Length.LengthSum [first, literal 1]))
          [2] 3 3

        propagationProblem <- adversarialWideConstantZeroProblemWithContract 3
          $ source $ Length.LengthAll
              [ Length.LengthAtMost
                  (Length.LengthMaximum first second) third
              , Length.LengthAtMost third $ literal 4
              ]
        propagationReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
            propagationProblem $ validate propagationProblem
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
          propagationReceipt [4, 4, 4] 125 55
            Evaluate.ProviderIndependentFiniteSpineModel
    , testCase
        "return the first root-extrema-derived replay counterexample" $ do
        let input = Length.LengthVariable $ Length.LengthInput 0
            result = Length.LengthVariable Length.LengthResult
            source = contractWith
              (Length.LengthAtMost
                (Length.LengthMaximum input $ Length.LengthScale 2 input)
                $ Length.LengthLiteral 4)
              $ Length.LengthEqual result input
        problem <- adversarialConstantZeroProblem source
        counterexample <- case
            Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits problem of
          Left failure -> assertFailure
            ("root-extrema counterexample validation failed: " ++ show failure)
              >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainInapplicable reason) ->
            assertFailure
              ("root-extrema counterexample domain was inapplicable: "
                ++ show reason) >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
            "root-extrema counterexample fixture produced positive evidence"
              >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainCounterexample evidence) ->
            expectRight $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem problem)
              evidence
        Evaluate.validatedLengthCounterexampleInputs counterexample @?= [1]
        Evaluate.validatedLengthCounterexampleResult counterexample @?= 0
        Evaluate.validatedLengthCounterexampleBasis counterexample @?=
          Evaluate.ProviderIndependentFiniteSpineModel
    , testCase
        "keep equality symmetric while emitting only the necessary extrema direction" $
        do
          let input = Length.LengthVariable $ Length.LengthInput 0
              literal value = Length.LengthLiteral value
              maximumExpression = Length.LengthMaximum
                (Length.LengthSum [Length.LengthScale 2 input, literal 1])
                input
              minimumLeft =
                Length.LengthSum [Length.LengthScale 2 input, literal 1]
              minimumExpression = Length.LengthMinimum
                (Length.LengthSum [input, literal 5]) $ literal 7
              source precondition = contractWith precondition
                $ Length.LengthTruth True
              validate problem =
                Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                  Evaluate.defaultLengthEvaluationLimits
                  Evaluate.defaultLengthInputBoxLimits problem
              assertEstablished precondition maximums assignments applicable = do
                ...
                assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                  receipt maximums assignments applicable
                    Evaluate.ProviderIndependentFiniteSpineModel
              assertOneWay equality = do
                problem <- adversarialConstantZeroProblem $ source equality
                reason <-
                  expectLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainInapplicability
                    $ validate problem
                reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0

          mapM_ (\equality -> assertEstablished equality [2] 3 1)
            [ Length.LengthEqual maximumExpression $ literal 5
            , Length.LengthEqual (literal 5) maximumExpression
            ]
          mapM_ (\equality -> assertEstablished equality [3] 4 1)
            [ Length.LengthEqual minimumLeft minimumExpression
            , Length.LengthEqual minimumExpression minimumLeft
            ]
          mapM_ assertOneWay
            [ Length.LengthEqual
                (Length.LengthMinimum input $ literal 100) $ literal 5
            , Length.LengthEqual (literal 5)
                $ Length.LengthMinimum input $ literal 100
            , Length.LengthEqual
    ...
    , testCase
        "apply strict successors before cancellation and let contradictions beat missing coverage" $
        do
          let first = Length.LengthVariable $ Length.LengthInput 0
              literal value = Length.LengthLiteral value
              source precondition = contractWith precondition
              ...
              validate problem =
                Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                  Evaluate.defaultLengthEvaluationLimits
                  Evaluate.defaultLengthInputBoxLimits problem
          successorProblem <- adversarialConstantZeroProblem $ source
            $ Length.LengthNot $ Length.LengthAtMost
                (Length.LengthMinimum
                  (Length.LengthSum [first, literal 3])
                  $ literal 10)
                $ Length.LengthScale 2 first
          successorReceipt <-
            expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
              successorProblem $ validate successorProblem
          assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
            successorReceipt [2] 3 3
              Evaluate.ProviderIndependentFiniteSpineModel

          contradictionProblem <-
            adversarialWideConstantZeroProblemWithContract 2 $ source
              $ Length.LengthAtMost
                  (Length.LengthMaximum (literal 5) first)
                  $ literal 4
          contradictionReceipt <-
            expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
              contradictionProblem $ validate contradictionProblem
          assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
            contradictionReceipt [0, 0] 1 0
              Evaluate.ProviderIndependentFiniteSpineModel
    , testCase "preserve literal and root-quotient predecessor projections" $ do
        let input = Length.LengthVariable $ Length.LengthInput 0
            literal value = Length.LengthLiteral value
            source precondition = contractWith precondition
              $ Length.LengthTruth True
        ...
        mapM_ (\precondition -> do
            ...
            predecessor <-
              expectValidatedLengthStrictRelationalPositiveAffineQuotientApplicableDomain
                problem $ validatePredecessor problem
            successor <-
              expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                problem $ validate problem
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainInclusiveMaximums
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientApplicableDomainInclusiveMaximums
                predecessor
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainAssignmentCount
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientApplicableDomainAssignmentCount
                predecessor
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainApplicableAssignmentCount
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientApplicableDomainApplicableAssignmentCount
                predecessor
    ...
    , testCase
        "ignore disjunctive orientations and every excluded extrema shape atomically" $
        do
          let input = Length.LengthVariable $ Length.LengthInput 0
              literal value = Length.LengthLiteral value
              makeMaximum = Length.LengthMaximum
              makeMinimum = Length.LengthMinimum
              source precondition = contractWith precondition
                $ Length.LengthTruth True
              validate problem =
                Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                  ...
                  Evaluate.defaultLengthInputBoxLimits problem
              excluded =
                [ Length.LengthAtMost input $ makeMaximum input $ literal 4
                , Length.LengthAtMost (makeMinimum input $ literal 4) $ literal 4
                , Length.LengthNot $ Length.LengthAtMost
                    (makeMaximum input $ literal 4) $ literal 4
                , Length.LengthNot $ Length.LengthAtMost input
                    $ makeMinimum input $ literal 4
                , Length.LengthAtMost
                    (makeMaximum input $ literal 1)
                    $ makeMinimum input $ literal 4
                , Length.LengthEqual
                    (makeMaximum input $ literal 1)
                    $ makeMinimum input $ literal 4
                , Length.LengthAtMost
                    (Length.LengthSum
                      [makeMaximum input $ literal 1, literal 1])
                    $ literal 4
                , Length.LengthAtMost
                    (makeMaximum input
                      $ Length.LengthMonus input $ literal 1)
                    $ literal 4
                , Length.LengthAtMost
                    (Length.LengthQuotient 2 input)
                    $ makeMinimum input $ literal 4
                , Length.LengthNot $ Length.LengthEqual
                    (makeMaximum input $ literal 1) $ literal 4
                ]
          mapM_ (\precondition -> do
              problem <- adversarialConstantZeroProblem $ source precondition
              reason <-
          ...
          let first = Length.LengthVariable $ Length.LengthInput 0
              second = Length.LengthVariable $ Length.LengthInput 1
              third = Length.LengthVariable $ Length.LengthInput 2
          naryProblem <- adversarialWideConstantZeroProblemWithContract 3
            $ source $ Length.LengthAtMost
                (makeMaximum (makeMaximum first second) third)
                $ literal 4
          naryReason <-
            expectLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainInapplicability
              $ validate naryProblem
          naryReason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
    , testCase "retain value, Cartesian-product, and replay precedence" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            result = Length.LengthVariable Length.LengthResult
            literal value = Length.LengthLiteral value
            bound variable maximumValue = Length.LengthAtMost
              (Length.LengthMaximum variable $ literal 0)
        ...
        binary <- adversarialBinaryConstantZeroProblem
          $ source (bound second 1) $ Length.LengthTruth True
        narrow <- inputBoxLimits 1 16
        width <- evaluateWithin
          $ validate (error "width rejection demanded evaluation limits")
              narrow binary
        ...
        oversized <- adversarialConstantZeroProblem
          $ source (bound first 8) $ Length.LengthTruth True
        assertLeft
          (Evaluate.LengthApplicableDomainInputBoxValidationRejected
            $ Evaluate.LengthInputBoxMaximumValueRejected 0
            $ Evaluate.LengthEvaluationValueBitLimitExceeded
        ...
        productProblem <- adversarialBinaryConstantZeroProblem $ source
          (Length.LengthAll [bound first 1, bound second 1])
          $ Length.LengthTruth True
        threeAssignments <- inputBoxLimits 2 3
        assertLeft
          (Evaluate.LengthApplicableDomainInputBoxValidationRejected
            $ Evaluate.LengthInputBoxAssignmentLimitExceeded 3 4)
          $ validate (evaluationLimitsWith 1 0)
              threeAssignments productProblem

        let isZero = Length.LengthEqual first $ literal 0
        replayProblem <- adversarialConstantZeroProblem $ source
          (bound first 0)
          $ Length.LengthEqual result
          $ Length.LengthIf isZero (literal 2) (literal 1)
        assertLeft
          (Evaluate.LengthApplicableDomainInputBoxValidationRejected
            $ Evaluate.LengthInputBoxAssignmentEvaluationRejected 0
            $ Evaluate.LengthEvaluationValueBitLimitExceeded
                Evaluate.LengthIntermediateValue 1 2)
          $ validate (evaluationLimitsWith 1 1)
              Evaluate.defaultLengthInputBoxLimits replayProblem
    , testCase
        "associate nominal scalar evidence without changing predecessor query identity" $
        do
          Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationSchemaTag
            @?= asciiBytes
              "finite-list-spine-length/strict-relational-positive-affine-quotient-root-extrema-precondition-domain-establishment/v1"
          assertBool "root-extrema validation reused the quotient predecessor schema"
            $ Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationSchemaTag
                /= Evaluate.lengthStrictRelationalPositiveAffineQuotientApplicableDomainValidationSchemaTag
          let input = Length.LengthVariable $ Length.LengthInput 0
              precondition = Length.LengthAtMost
          ...
          evidence <- case directValidation of
            Right (Evaluate.LengthApplicableDomainEstablished established) ->
              pure established
            _ -> assertFailure "root-extrema scalar validation lost positive evidence"
              >> error "unreachable"
          stale <- adversarialConstantZeroProblem
            $ contractWith
                (Length.LengthAtMost
                  (Length.LengthMaximum input $ Length.LengthLiteral 0)
                  $ Length.LengthLiteral 1)
                $ Length.LengthTruth True
          assertLeft SemanticProblem.ReplayEncodingFingerprintMismatch
            $ SemanticProblem.replayBehavioralEvidence
          ...
          let fingerprintBefore = SMTLib.lengthSMTLibQueryFingerprint query
              ...
              symbolsBefore = SMTLib.lengthSMTLibQueryInputSymbols query
              valuesBefore = SMTLib.lengthSMTLibQueryInputValueRequestBytes query
          predecessorQuery <- expectRight
            $ SMTLib.validateLengthSMTLibQueryStrictRelationalPositiveAffineQuotientApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits query
          case predecessorQuery of
            Evaluate.LengthApplicableDomainInapplicable reason ->
              reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
            _ -> assertFailure
              "quotient predecessor query validation accepted an extrema-only bound"
          queryValidation <- expectRight
            $ SMTLib.validateLengthSMTLibQueryStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits query
          case queryValidation of
            Evaluate.LengthApplicableDomainEstablished queryReceipt ->
              queryReceipt @?= receipt
            Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
              $ "root-extrema scalar query validation was inapplicable: "
                  ++ show reason
            Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
              "root-extrema scalar query validation found a counterexample"
          SMTLib.lengthSMTLibQueryFingerprint query @?= fingerprintBefore
          SMTLib.lengthSMTLibQueryCheckBytes query @?= checkBefore
          SMTLib.lengthSMTLibQueryInputSymbols query @?= symbolsBefore
          SMTLib.lengthSMTLibQueryInputValueRequestBytes query @?= valuesBefore
    , testCase "keep product evidence nominal and preserve pair query identity" $ do
        Evaluate.lengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationSchemaTag
          @?= asciiBytes
        ...
        assertBool "root-extrema product validation reused the scalar schema"
          $ Evaluate.lengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationSchemaTag
              /= Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthSpinePairInput 0
            precondition = Length.LengthAtMost
        ...
        problem <- adversarialInputAndZeroSpinePairProblem source
        let directValidation =
              Evaluate.validateLengthSpinePairProblemStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits problem
        receipt <-
          expectValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
            problem directValidation
        assertValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
          receipt [2] 3 3 Evaluate.ProviderIndependentFiniteSpineModel
        force receipt `seq` pure ()
        productEvidence <- case directValidation of
          Right (Evaluate.LengthApplicableDomainEstablished established) ->
            pure established
          _ -> assertFailure "root-extrema product validation lost positive evidence"
            >> error "unreachable"
        scalar <- adversarialConstantZeroProblem
          $ contractWith
              (Length.LengthAtMost
        ...
        let forgedScalarEvidence = unsafeCoerce productEvidence
              :: SemanticProblem.BehavioralEvidence
                  Length.FiniteListSpineLengthV1
                  Evaluate.ValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
        assertLeft SemanticProblem.ReplayDomainMismatch
          $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem scalar)
              forgedScalarEvidence
        ...
        let fingerprintBefore =
            ...
            valuesBefore =
              SMTLib.lengthSpinePairSMTLibQueryInputValueRequestBytes query
        queryValidation <- expectRight
          $ SMTLib.validateLengthSpinePairSMTLibQueryStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits query
        case queryValidation of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            $ "root-extrema product query validation was inapplicable: "
                ++ show reason
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "root-extrema product query validation found a counterexample"
        SMTLib.lengthSpinePairSMTLibQueryFingerprint query @?=
          fingerprintBefore
        SMTLib.lengthSpinePairSMTLibQueryCheckBytes query @?= checkBefore
        SMTLib.lengthSpinePairSMTLibQueryInputSymbols query @?= symbolsBefore
...
strictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationTests =
  testGroup
    "strict relational positive-affine quotient root-extrema monus applicable-domain validation"
    [ testCase "apply all four directed monus laws and propagate their rules" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            third = Length.LengthVariable $ Length.LengthInput 2
            literal value = Length.LengthLiteral value
            source precondition = contractWith precondition
              $ Length.LengthTruth True
            validate problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits problem
            assertUnary precondition maximums assignments applicable = do
              problem <- adversarialConstantZeroProblem $ source precondition
              receipt <-
                expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                  problem $ validate problem
              assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                receipt maximums assignments applicable
                  Evaluate.ProviderIndependentFiniteSpineModel

        -- (2x + 1) monus x <= 3 gives 2x + 1 <= x + 3.
        assertUnary
          (Length.LengthAtMost
            (Length.LengthMonus
              (Length.LengthSum
        ...
        -- x + 1 <= 5 monus x is uniformly positive on the left and gives
        -- x + (x + 1) <= 5.
        assertUnary
          (Length.LengthAtMost
            (Length.LengthSum [first, literal 1])
        ...
        -- not (5 monus x <= 0) gives x + 1 <= 5.
        assertUnary
          (Length.LengthNot $ Length.LengthAtMost
            (Length.LengthMonus (literal 5) first) $ literal 0)
          [4] 5 5

        -- not (5 <= 2x monus 1) atomically gives 1 <= 5 and
        -- 2x + 1 <= 1 + 5.
        assertUnary
          (Length.LengthNot $ Length.LengthAtMost (literal 5)
            $ Length.LengthMonus (Length.LengthScale 2 first)
                (literal 1))
          [2] 3 3
        ...
        propagationProblem <- adversarialWideConstantZeroProblemWithContract 3
          $ source $ Length.LengthAll
              [ Length.LengthAtMost
                  (Length.LengthMonus first second) $ literal 0
              , Length.LengthAtMost second third
              , Length.LengthAtMost third $ literal 4
              ]
        propagationReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
            propagationProblem $ validate propagationProblem
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
          propagationReceipt [4, 4, 4] 125 35
            Evaluate.ProviderIndependentFiniteSpineModel
    , testCase
        "guard C <= monus by intrinsic positivity and retain zero semantics" $
        do
          let input = Length.LengthVariable $ Length.LengthInput 0
              literal value = Length.LengthLiteral value
              monus = Length.LengthMonus (literal 5) input
              source precondition = contractWith precondition
                $ Length.LengthTruth True
              validate problem =
                Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                  Evaluate.defaultLengthEvaluationLimits
                  Evaluate.defaultLengthInputBoxLimits problem
              assertMissing precondition = do
                problem <- adversarialConstantZeroProblem $ source precondition
                reason <-
                  expectLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainInapplicability
                    $ validate problem
                reason @?=
                  Evaluate.LengthApplicableDomainInputUpperBoundMissing 0

          assertMissing $ Length.LengthAtMost (literal 0) monus
          assertMissing $ Length.LengthAtMost input monus
          assertMissing $ Length.LengthAll
            [ Length.LengthAtMost (literal 1) input
            , Length.LengthAtMost input monus
            ]
    , testCase "return the first monus-derived replay counterexample" $ do
        let input = Length.LengthVariable $ Length.LengthInput 0
            result = Length.LengthVariable Length.LengthResult
            precondition = Length.LengthAtMost
              (Length.LengthMonus
        ...
        counterexample <- case
            Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
              ...
              Evaluate.defaultLengthInputBoxLimits problem of
          Left failure -> assertFailure
            ("monus counterexample validation failed: " ++ show failure)
              >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainInapplicable reason) ->
            assertFailure
              ("monus counterexample domain was inapplicable: "
                ++ show reason) >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
            "monus counterexample fixture produced positive evidence"
              >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainCounterexample evidence) ->
            expectRight $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem problem)
        ...
        Evaluate.validatedLengthCounterexampleBasis counterexample @?=
          Evaluate.ProviderIndependentFiniteSpineModel
    , testCase
        "keep monus equality symmetric and gate only its sufficient half" $ do
        let input = Length.LengthVariable $ Length.LengthInput 0
            literal value = Length.LengthLiteral value
            monus = Length.LengthMonus (literal 5) input
            orientations opposite =
              [ Length.LengthEqual monus opposite
              , Length.LengthEqual opposite monus
              ]
            source precondition = contractWith precondition
              $ Length.LengthTruth True
            validate problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits problem
            assertEstablished precondition = do
              problem <- adversarialConstantZeroProblem $ source precondition
              receipt <-
                expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                  problem $ validate problem
            ...
            assertMissing precondition = do
              ...
              reason @?=
                Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
            assertNecessary precondition upper = do
              problem <- adversarialWideConstantZeroProblemWithContract 2
                $ source $ Length.LengthAll
                    [precondition, Length.LengthAtMost input $ literal upper]
              receipt <-
                expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                  problem $ validate problem
              assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
        ...
        mapM_ assertEstablished $ orientations $ literal 1
        mapM_ assertMissing $ orientations $ literal 0
        mapM_ assertMissing $ orientations input
        mapM_ (`assertNecessary` 4) $ orientations $ literal 0
        mapM_ (`assertNecessary` 2) $ orientations input
    , testCase
        "apply strict successors before cancellation and emit boundary rules atomically" $
        do
          let input = Length.LengthVariable $ Length.LengthInput 0
              literal value = Length.LengthLiteral value
              source precondition = contractWith precondition
          ...
          successorProblem <- adversarialConstantZeroProblem $ source
            $ Length.LengthNot $ Length.LengthAtMost
                (Length.LengthMonus (literal 5) input) $ literal 0
          successorReceipt <-
            expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
              successorProblem $ validate successorProblem
          assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
            successorReceipt [4] 5 5
              Evaluate.ProviderIndependentFiniteSpineModel

          contradictionProblem <-
            adversarialWideConstantZeroProblemWithContract 2 $ source
              $ Length.LengthNot $ Length.LengthAtMost (literal 0)
                  $ Length.LengthMonus input $ literal 1
          contradictionReceipt <-
            expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
              contradictionProblem $ validate contradictionProblem
          assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
            contradictionReceipt [0, 0] 1 0
              Evaluate.ProviderIndependentFiniteSpineModel
    , testCase "preserve every root-extrema predecessor projection" $ do
        let input = Length.LengthVariable $ Length.LengthInput 0
            literal value = Length.LengthLiteral value
            source precondition = contractWith precondition
              $ Length.LengthTruth True
        ...
        mapM_ (\precondition -> do
            ...
            successor <-
              expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                problem $ validate problem
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainInclusiveMaximums
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainInclusiveMaximums
                predecessor
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainAssignmentCount
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainAssignmentCount
                predecessor
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainApplicableAssignmentCount
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainApplicableAssignmentCount
                predecessor
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainBasis
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainBasis
                predecessor)
          [ Length.LengthAtMost input $ literal 2
    ...
    , testCase "ignore every unsupported monus mixture atomically" $ do
        let input = Length.LengthVariable $ Length.LengthInput 0
            literal value = Length.LengthLiteral value
            monus left right = Length.LengthMonus left right
            source precondition = contractWith precondition
              $ Length.LengthTruth True
            validate problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
            ...
            excluded =
              [ Length.LengthAtMost
                  (monus input $ literal 1)
                  $ monus (literal 5) input
              , Length.LengthEqual
                  (monus input $ literal 1)
                  $ monus (literal 5) input
              , Length.LengthAtMost
                  (monus (monus input $ literal 1) $ literal 1)
                  $ literal 4
              , Length.LengthAtMost
                  (Length.LengthSum
                    [monus input $ literal 1, literal 1])
                  $ literal 4
              , Length.LengthAtMost
                  (monus (Length.LengthQuotient 2 input) $ literal 1)
                  $ literal 4
              , Length.LengthAtMost
                  (Length.LengthQuotient 2 input)
                  $ monus (literal 5) input
              ...
              , Length.LengthAtMost
                  ...
                  $ monus (literal 5) input
              , Length.LengthNot $ Length.LengthEqual
                  (monus input $ literal 1) $ literal 4
              ]
        mapM_ (\precondition -> do
            problem <- adversarialConstantZeroProblem $ source precondition
            reason <-
            ...
            reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0)
          excluded
    , testCase "retain width, value, product, and replay precedence" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            result = Length.LengthVariable Length.LengthResult
            literal value = Length.LengthLiteral value
            bound variable maximumValue = Length.LengthAtMost
              (Length.LengthMonus
                (Length.LengthSum [variable, literal 1]) $ literal 1)
              $ literal maximumValue
            source precondition postcondition =
              contractWith precondition postcondition
            validate limits box problem =
    ...
    , testCase
        "associate nominal scalar evidence without changing predecessor query identity" $
        do
          Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationSchemaTag
            @?= asciiBytes
          ...
          assertBool "monus validation reused the root-extrema schema"
            $ Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationSchemaTag
                /= Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidationSchemaTag
          let input = Length.LengthVariable $ Length.LengthInput 0
              precondition = Length.LengthAtMost
          ...
          evidence <- case directValidation of
            Right (Evaluate.LengthApplicableDomainEstablished established) ->
              pure established
            _ -> assertFailure "monus scalar validation lost positive evidence"
              >> error "unreachable"
          stale <- adversarialConstantZeroProblem
            $ contractWith
                (Length.LengthAtMost
          ...
          case predecessorQuery of
            Evaluate.LengthApplicableDomainInapplicable reason ->
              reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
            _ -> assertFailure
              "root-extrema predecessor query validation accepted a monus-only bound"
          queryValidation <- expectRight
            $ SMTLib.validateLengthSMTLibQueryStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits query
          case queryValidation of
            Evaluate.LengthApplicableDomainEstablished queryReceipt ->
              queryReceipt @?= receipt
            Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
              $ "monus scalar query validation was inapplicable: "
                  ++ show reason
            Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
              "monus scalar query validation found a counterexample"
          SMTLib.lengthSMTLibQueryFingerprint query @?= fingerprintBefore
          SMTLib.lengthSMTLibQueryCheckBytes query @?= checkBefore
          SMTLib.lengthSMTLibQueryInputSymbols query @?= symbolsBefore
          SMTLib.lengthSMTLibQueryInputValueRequestBytes query @?= valuesBefore
    , testCase "keep product evidence nominal and preserve pair query identity" $ do
        Evaluate.lengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationSchemaTag
          @?= asciiBytes
        ...
        assertBool "monus product validation reused the scalar schema"
          $ Evaluate.lengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationSchemaTag
              /= Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthSpinePairInput 0
            precondition = Length.LengthAtMost
        ...
        problem <- adversarialInputAndZeroSpinePairProblem source
        let directValidation =
              Evaluate.validateLengthSpinePairProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits problem
        receipt <-
          expectValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
            problem directValidation
        assertValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
          receipt [2] 3 3 Evaluate.ProviderIndependentFiniteSpineModel
        force receipt `seq` pure ()
        productEvidence <- case directValidation of
          Right (Evaluate.LengthApplicableDomainEstablished established) ->
            pure established
          _ -> assertFailure "monus product validation lost positive evidence"
            >> error "unreachable"
        scalar <- adversarialConstantZeroProblem
          $ contractWith
              (Length.LengthAtMost
        ...
        let forgedScalarEvidence = unsafeCoerce productEvidence
              :: SemanticProblem.BehavioralEvidence
                  Length.FiniteListSpineLengthV1
                  Evaluate.ValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
        assertLeft SemanticProblem.ReplayDomainMismatch
          $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem scalar)
              forgedScalarEvidence
        ...
        case predecessorQuery of
          Evaluate.LengthApplicableDomainInapplicable reason ->
            reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
          _ -> assertFailure
            "root-extrema pair query validation accepted a monus-only bound"
        queryValidation <- expectRight
          $ SMTLib.validateLengthSpinePairSMTLibQueryStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits query
        case queryValidation of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            $ "monus product query validation was inapplicable: "
                ++ show reason
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "monus product query validation found a counterexample"
        SMTLib.lengthSpinePairSMTLibQueryFingerprint query @?=
          fingerprintBefore
        SMTLib.lengthSpinePairSMTLibQueryCheckBytes query @?= checkBefore
        SMTLib.lengthSpinePairSMTLibQueryInputSymbols query @?= symbolsBefore
...
strictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainValidationTests =
  testGroup
    "strict relational positive-affine quotient root-extrema monus Boolean finite-union applicable-domain validation"
    [ testCase "expand De Morgan DNF and negated equality exactly" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            literal value = Length.LengthLiteral value
            bound variable maximumValue =
              Length.LengthAtMost variable $ literal maximumValue
            disjoin formulas = Length.LengthNot
              $ Length.LengthAll $ map Length.LengthNot formulas
            generalPrecondition = Length.LengthAll
              [ bound first 4
              , bound second 4
              , disjoin [bound first 1, bound second 1]
              , disjoin [bound first 3, bound second 3]
              ]
            source precondition = contractWith precondition
              $ Length.LengthTruth True
            validate limits box problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                Evaluate.defaultLengthEvaluationLimits box limits problem
        general <- adversarialBinaryConstantZeroProblem
          $ source generalPrecondition
        generalLimits <- booleanFiniteUnionLimits 4 4 4 2 20
        generalBox <- inputBoxLimits 2 16
        generalReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
            general $ validate generalLimits generalBox general
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          generalReceipt [[1, 4], [4, 1]] 2 20 16 16
            Evaluate.ProviderIndependentFiniteSpineModel

        let input = Length.LengthVariable $ Length.LengthInput 0
            inequality = Length.LengthAll
              [ bound input 3
              , Length.LengthNot $ Length.LengthEqual input $ literal 1
              ]
        unequal <- adversarialConstantZeroProblem $ source inequality
        unequalLimits <- booleanFiniteUnionLimits 2 2 2 1 4
        unequalBox <- inputBoxLimits 1 4
        unequalReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
            unequal $ validate unequalLimits unequalBox unequal
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          unequalReceipt [[3]] 1 4 4 3
            Evaluate.ProviderIndependentFiniteSpineModel
    , testCase
        "canonicalize branch and origin-box antichains without a hull" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            literal value = Length.LengthLiteral value
            bound variable maximumValue =
            ...
            validate limits box problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                Evaluate.defaultLengthEvaluationLimits box limits problem
            p = Length.LengthAll [bound first 1, bound second 2]
            q = bound first 0
            absorbed = Length.LengthAll
              [disjoin [p, q], disjoin [p, Length.LengthNot q]]
        absorbedProblem <- adversarialBinaryConstantZeroProblem
          $ source absorbed
        absorbedLimits <- booleanFiniteUnionLimits 4 2 2 1 6
        absorbedBox <- inputBoxLimits 2 6
        absorbedReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
            absorbedProblem
            $ validate absorbedLimits absorbedBox absorbedProblem
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          absorbedReceipt [[1, 2]] 1 6 6 6
            Evaluate.ProviderIndependentFiniteSpineModel

        let firstBox = Length.LengthAll
              [bound first 1, bound second 3]
            secondBox = Length.LengthAll
              [bound first 3, bound second 1]
            containedBox = Length.LengthAll
              [bound first 1, bound second 2]
            noHull = disjoin [containedBox, secondBox, firstBox]
        noHullProblem <- adversarialBinaryConstantZeroProblem $ source noHull
        noHullLimits <- booleanFiniteUnionLimits 3 2 2 2 16
        noHullBox <- inputBoxLimits 2 12
        noHullReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
            noHullProblem $ validate noHullLimits noHullBox noHullProblem
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          noHullReceipt [[1, 3], [3, 1]] 2 16 12 12
            Evaluate.ProviderIndependentFiniteSpineModel
    , testCase "enforce every work cap at exact max plus one" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            second = Length.LengthVariable $ Length.LengthInput 1
            literal value = Length.LengthLiteral value
            bound variable maximumValue =
            ...
            validate limits box problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                (error "finite-union preparation demanded evaluation limits")
                box limits problem
            p = Length.LengthAll [bound first 1, bound second 2]
            q = bound first 0
            absorbed = Length.LengthAll
              [disjoin [p, q], disjoin [p, Length.LengthNot q]]
        branchProblem <- adversarialBinaryConstantZeroProblem $ source absorbed
        branchLimits <- booleanFiniteUnionLimits 3 64 4096 256 262144
        branchResult <- evaluateWithin $ validate branchLimits
          Evaluate.defaultLengthInputBoxLimits branchProblem
        assertLeft
          (Evaluate.LengthBooleanFiniteUnionGeneratedBranchLimitExceeded 3 4)
          branchResult

        equalityProblem <- adversarialConstantZeroProblem $ source
          $ Length.LengthEqual first $ literal 2
        ruleLimits <- booleanFiniteUnionLimits 1 1 4096 256 262144
        ruleResult <- evaluateWithin $ validate ruleLimits
          Evaluate.defaultLengthInputBoxLimits equalityProblem
        assertLeft
          (Evaluate.LengthBooleanFiniteUnionRuleLimitExceeded 0 1 2)
          ruleResult

        chainProblem <- adversarialBinaryConstantZeroProblem $ source
          $ Length.LengthAll
              [Length.LengthAtMost first second, bound second 1]
        closureLimits <- booleanFiniteUnionLimits 1 2 1 256 262144
        closureResult <- evaluateWithin $ validate closureLimits
          Evaluate.defaultLengthInputBoxLimits chainProblem
        assertLeft
        ...
        let firstBox = Length.LengthAll
            ...
            secondBox = Length.LengthAll
              [bound first 3, bound second 1]
            noHull = disjoin [secondBox, firstBox]
        unionProblem <- adversarialBinaryConstantZeroProblem $ source noHull
        boxLimits <- booleanFiniteUnionLimits 2 2 2 1 16
        boxResult <- evaluateWithin $ validate boxLimits
          Evaluate.defaultLengthInputBoxLimits unionProblem
        assertLeft
          (Evaluate.LengthBooleanFiniteUnionRetainedBoxLimitExceeded 1 2)
          boxResult

        visitLimits <- booleanFiniteUnionLimits 2 2 2 2 15
        uniqueEleven <- inputBoxLimits 2 11
        assertLeft
          (Evaluate.LengthBooleanFiniteUnionAssignmentVisitLimitExceeded 15 16)
          $ Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
              Evaluate.defaultLengthEvaluationLimits uniqueEleven
              visitLimits unionProblem

        exactVisitLimits <- booleanFiniteUnionLimits 2 2 2 2 16
        assertLeft
          (Evaluate.LengthBooleanFiniteUnionAssignmentLimitExceeded 11 12)
          $ Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
              Evaluate.defaultLengthEvaluationLimits uniqueEleven
              exactVisitLimits unionProblem
    , testCase "distinguish the empty union from the nullary singleton" $ do
        let input = Length.LengthVariable $ Length.LengthInput 0
            bound = Length.LengthAtMost input $ Length.LengthLiteral 0
            complement = Length.LengthAll [bound, Length.LengthNot bound]
            source precondition = contractWith precondition
              $ Length.LengthTruth True
            validate evaluation problem =
              Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                evaluation Evaluate.defaultLengthInputBoxLimits
                Evaluate.defaultLengthBooleanFiniteUnionLimits problem
        empty <- adversarialConstantZeroProblem $ source complement
        emptyReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
            empty $ validate
              (error "the empty finite union demanded evaluation limits") empty
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          emptyReceipt [] 0 0 0 0
            Evaluate.ProviderIndependentFiniteSpineModel

        nullary <- adversarialZeroInputProblem
          $ source $ Length.LengthTruth True
        nullaryReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
            nullary $ validate Evaluate.defaultLengthEvaluationLimits nullary
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          nullaryReceipt [[]] 1 1 1 1
            Evaluate.ProviderIndependentFiniteSpineModel

        falseNullary <- adversarialZeroInputProblem
          $ source $ Length.LengthTruth False
        falseReceipt <-
          expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
            falseNullary $ validate
              (error "the false nullary union demanded evaluation limits")
              falseNullary
        assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          falseReceipt [] 0 0 0 0
            Evaluate.ProviderIndependentFiniteSpineModel
        force emptyReceipt `seq` force nullaryReceipt `seq`
          force falseReceipt `seq` pure ()
    , testCase
        "replay the unique union globally and return its first lexicographic counterexample" $
        do
          let first = Length.LengthVariable $ Length.LengthInput 0
              second = Length.LengthVariable $ Length.LengthInput 1
              third = Length.LengthVariable $ Length.LengthInput 2
              ...
              disjoin formulas = Length.LengthNot
                $ Length.LengthAll $ map Length.LengthNot formulas
              firstBox = Length.LengthAll
                [bound first 1, bound second 1, bound third 3]
              secondBox = Length.LengthAll
                [bound first 1, bound second 3, bound third 1]
              precondition = disjoin [secondBox, firstBox]
          ...
          let validation =
                Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                  ...
                  Evaluate.defaultLengthInputBoxLimits
                  Evaluate.defaultLengthBooleanFiniteUnionLimits problem
          counterexample <- case validation of
            Left failure -> assertFailure
              ("Boolean finite-union validation failed: " ++ show failure)
                >> error "unreachable"
            Right (Evaluate.LengthApplicableDomainInapplicable reason) ->
              assertFailure
                ("Boolean finite union was inapplicable: " ++ show reason)
                >> error "unreachable"
            Right (Evaluate.LengthApplicableDomainEstablished _) ->
              assertFailure "the violating finite union produced evidence"
                >> error "unreachable"
            Right (Evaluate.LengthApplicableDomainCounterexample evidence) ->
              expectRight $ SemanticProblem.replayBehavioralEvidence
                (LengthProblem.checkedLengthProblemBehavioralProblem problem)
          ...
          Evaluate.validatedLengthCounterexampleBasis counterexample @?=
            Evaluate.ProviderIndependentFiniteSpineModel
    , testCase "retain the full fixed failure precedence" $ do
        let first = Length.LengthVariable $ Length.LengthInput 0
            result = Length.LengthVariable Length.LengthResult
            literal value = Length.LengthLiteral value
            bound variable maximumValue =
        ...
        narrow <- inputBoxLimits 1 16
        widthProblem <- adversarialBinaryConstantZeroProblem $ source
          (Length.LengthAll
            [bound first 0, Length.LengthNot $ bound first 0])
          $ Length.LengthTruth True
        ...
        let guarded = Length.LengthAtMost
              ...
              $ literal 3
        missingProblem <- adversarialConstantZeroProblem $ source guarded
          $ Length.LengthTruth True
        zeroWork <- booleanFiniteUnionLimits 1 0 0 0 0
        missing <-
          expectLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainInapplicability
            $ validate
                ...
                Evaluate.defaultLengthInputBoxLimits zeroWork missingProblem
        missing @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0

        maximumProblem <- adversarialConstantZeroProblem $ source
          (bound first 3) $ Length.LengthTruth True
        ...
        zeroAssignments <- inputBoxLimits 1 0
        assertLeft
          (Evaluate.LengthBooleanFiniteUnionAssignmentLimitExceeded 0 1)
          $ validate (evaluationLimitsWith 0 0) zeroAssignments oneVisit
              replayProblem
        oneAssignment <- inputBoxLimits 1 1
        assertLeft
          ...
          $ validate (evaluationLimitsWith 0 0) oneAssignment oneVisit
              replayProblem
    , testCase "preserve every monus predecessor projection literally" $ do
        Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidationSchemaTag
          @?= asciiBytes
            "finite-list-spine-length/strict-relational-positive-affine-quotient-root-extrema-monus-precondition-domain-establishment/v1"
        ...
        let input = Length.LengthVariable $ Length.LengthInput 0
            ...
            preconditions =
              ...
              , Length.LengthAtMost
                  (Length.LengthMonus
                    (Length.LengthSum
                      [Length.LengthScale 2 input, literal 1]) input)
                  $ literal 3
              ]
        mapM_ (\precondition -> do
            problem <- adversarialConstantZeroProblem $ source precondition
            ...
            successor <-
              expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                problem $ validateSuccessor problem
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainInclusiveMaximumBoxes
              ...
              [Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainInclusiveMaximums
                predecessor]
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainBoxCount
                successor @?= 1
            ...
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainApplicableAssignmentCount
              ...
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainApplicableAssignmentCount
                predecessor
            Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainBasis
                successor @?=
              Evaluate.validatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainBasis
                predecessor)
          preconditions
    , testCase
        "associate scalar finite-union evidence without changing query identity" $
        do
          let first = Length.LengthVariable $ Length.LengthInput 0
              second = Length.LengthVariable $ Length.LengthInput 1
              literal value = Length.LengthLiteral value
              ...
              disjoin formulas = Length.LengthNot
                $ Length.LengthAll $ map Length.LengthNot formulas
              box leftMaximum rightMaximum = Length.LengthAll
                [bound first leftMaximum, bound second rightMaximum]
              precondition = disjoin [box 3 1, box 1 3]
              source = contractWith precondition $ Length.LengthTruth True
          problem <- adversarialBinaryConstantZeroProblem source
          let directValidation =
                Evaluate.validateLengthProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          ...
          (evidence, receipt) <- case directValidation of
            Right (Evaluate.LengthApplicableDomainEstablished established) ->
              do
                ...
                pure (established, replayed)
            Left failure -> assertFailure
              ("scalar finite-union validation failed: " ++ show failure)
                >> error "unreachable"
            Right (Evaluate.LengthApplicableDomainInapplicable reason) ->
              assertFailure
                ("scalar finite-union validation was inapplicable: "
                  ++ show reason) >> error "unreachable"
            Right (Evaluate.LengthApplicableDomainCounterexample _) ->
              assertFailure
                "scalar finite-union validation found a counterexample"
                  >> error "unreachable"
          assertValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          ...
          stale <- adversarialBinaryConstantZeroProblem
            $ contractWith (disjoin [box 2 1, box 1 2])
                $ Length.LengthTruth True
          assertLeft SemanticProblem.ReplayEncodingFingerprintMismatch
            $ SemanticProblem.replayBehavioralEvidence
                (LengthProblem.checkedLengthProblemBehavioralProblem stale)
          ...
          case predecessor of
            Evaluate.LengthApplicableDomainInapplicable reason ->
              reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
            _ -> assertFailure
              "the single-box predecessor accepted a Boolean union"
          associated <- expectRight
            $ SMTLib.validateLengthSMTLibQueryStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits
          ...
          case associated of
            Evaluate.LengthApplicableDomainEstablished queryReceipt ->
              queryReceipt @?= receipt
            Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
              ("scalar Boolean query was inapplicable: " ++ show reason)
            Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
              "scalar Boolean query produced a counterexample"
          SMTLib.lengthSMTLibQueryFingerprint query @?= fingerprintBefore
          SMTLib.lengthSMTLibQueryCheckBytes query @?= checkBefore
          SMTLib.lengthSMTLibQueryInputSymbols query @?= symbolsBefore
          SMTLib.lengthSMTLibQueryInputValueRequestBytes query @?= valuesBefore
    , testCase "keep product evidence nominal and pair query bytes unchanged" $ do
        Evaluate.lengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainValidationSchemaTag
          @?= asciiBytes
        ...
        assertBool "Boolean finite-union product validation reused scalar schema"
          $ Evaluate.lengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainValidationSchemaTag
              /= Evaluate.lengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainValidationSchemaTag
        let input = Length.LengthVariable $ Length.LengthSpinePairInput 0
            literal value = Length.LengthLiteral value
            bound maximumValue = Length.LengthAtMost input
              $ literal maximumValue
            disjoin formulas = Length.LengthNot
              $ Length.LengthAll $ map Length.LengthNot formulas
            precondition = disjoin [bound 3, bound 1]
            source = spinePairContractWith precondition
              $ Length.LengthAll
                  [ Length.LengthEqual
                      (pairResultVariable Length.LengthSpinePairFirst) input
        ...
        problem <- adversarialInputAndZeroSpinePairProblem source
        let directValidation =
              Evaluate.validateLengthSpinePairProblemStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
                Evaluate.defaultLengthEvaluationLimits
                Evaluate.defaultLengthInputBoxLimits
                Evaluate.defaultLengthBooleanFiniteUnionLimits problem
        (evidence, receipt) <- case directValidation of
          Right (Evaluate.LengthApplicableDomainEstablished established) ->
            do
              ...
              pure (established, replayed)
          Left failure -> assertFailure
            ("product finite-union validation failed: " ++ show failure)
              >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainInapplicable reason) ->
            assertFailure
              ("product finite-union validation was inapplicable: "
                ++ show reason) >> error "unreachable"
          Right (Evaluate.LengthApplicableDomainCounterexample _) ->
            assertFailure
              "product finite-union validation found a counterexample"
                >> error "unreachable"
        assertValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
          receipt [[3]] 1 4 4 4
            Evaluate.ProviderIndependentFiniteSpineModel
        force receipt `seq` pure ()
        ...
        scalar <- adversarialConstantZeroProblem
          $ contractWith
              (Length.LengthNot $ Length.LengthAll
                ...
                , Length.LengthNot $ Length.LengthAtMost
                    (Length.LengthVariable $ Length.LengthInput 0) $ literal 1
                ])
              $ Length.LengthTruth True
        let forgedScalarEvidence = unsafeCoerce evidence
              :: SemanticProblem.BehavioralEvidence
                  Length.FiniteListSpineLengthV1
                  Evaluate.ValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
        assertLeft SemanticProblem.ReplayDomainMismatch
          $ SemanticProblem.replayBehavioralEvidence
              (LengthProblem.checkedLengthProblemBehavioralProblem scalar)
              forgedScalarEvidence
        ...
        case predecessor of
          Evaluate.LengthApplicableDomainInapplicable reason ->
            reason @?= Evaluate.LengthApplicableDomainInputUpperBoundMissing 0
          _ -> assertFailure
            "the product single-box predecessor accepted a Boolean union"
        associated <- expectRight
          $ SMTLib.validateLengthSpinePairSMTLibQueryStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
              Evaluate.defaultLengthEvaluationLimits
              Evaluate.defaultLengthInputBoxLimits
              Evaluate.defaultLengthBooleanFiniteUnionLimits query
        case associated of
          Evaluate.LengthApplicableDomainEstablished queryReceipt ->
            queryReceipt @?= receipt
          Evaluate.LengthApplicableDomainInapplicable reason -> assertFailure
            ("product Boolean query was inapplicable: " ++ show reason)
          Evaluate.LengthApplicableDomainCounterexample{} -> assertFailure
            "product Boolean query produced a counterexample"
        SMTLib.lengthSpinePairSMTLibQueryFingerprint query @?=
          fingerprintBefore
        SMTLib.lengthSpinePairSMTLibQueryCheckBytes query @?= checkBefore
        SMTLib.lengthSpinePairSMTLibQueryInputSymbols query @?= symbolsBefore
...
type AdversarialLengthPositiveAffineApplicableDomainValidation =
  Either Evaluate.LengthApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteListSpineLengthV1
...
type AdversarialLengthSpinePairPositiveAffineApplicableDomainValidation =
  Either Evaluate.LengthSpinePairApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      ...
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
        Evaluate.ValidatedLengthSpinePairPositiveAffineApplicableDomain))

expectLengthPositiveAffineApplicableDomainInapplicability
  :: AdversarialLengthPositiveAffineApplicableDomainValidation
  -> IO Evaluate.LengthApplicableDomainInapplicability
expectLengthPositiveAffineApplicableDomainInapplicability validation =
  case validation of
    Left failure -> assertFailure
      ("unexpected positive-affine domain rejection: " ++ show failure)
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainInapplicable reason) -> pure reason
    Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
      "an inapplicable positive-affine domain produced a counterexample"
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
      "an inapplicable positive-affine domain produced positive evidence"
        >> error "unreachable"

expectValidatedLengthPositiveAffineApplicableDomain
  :: LengthProblem.CheckedLengthProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthPositiveAffineApplicableDomain
expectValidatedLengthPositiveAffineApplicableDomain problem validation =
  case validation of
    Left failure -> assertFailure
      ("unexpected positive-affine domain rejection: " ++ show failure)
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
      ("the positive-affine domain was not bounded: " ++ show reason)
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
      "the positive-affine domain unexpectedly contained a counterexample"
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
      $ SemanticProblem.replayBehavioralEvidence
          (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence

expectLengthPositiveAffineApplicableDomainCounterexample
  :: LengthProblem.CheckedLengthProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthCounterexample
expectLengthPositiveAffineApplicableDomainCounterexample problem validation =
  case validation of
    Left failure -> assertFailure
      ("unexpected positive-affine domain rejection: " ++ show failure)
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
      ("the violating positive-affine domain was inapplicable: " ++ show reason)
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
      "the violating positive-affine domain produced positive evidence"
        >> error "unreachable"
    Right (Evaluate.LengthApplicableDomainCounterexample evidence) ->
      expectRight $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence

expectValidatedLengthSpinePairPositiveAffineApplicableDomain
  :: LengthProblem.CheckedLengthSpinePairProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthSpinePairPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthSpinePairPositiveAffineApplicableDomain
expectValidatedLengthSpinePairPositiveAffineApplicableDomain problem
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected positive-affine product-domain rejection: " ++ show failure)
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the positive-affine product domain was not bounded: " ++ show reason)
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the positive-affine product domain unexpectedly contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthSpinePairProblemBehavioralProblem problem)
...
expectLengthSpinePairPositiveAffineApplicableDomainCounterexample
  ...
  -> AdversarialLengthSpinePairPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthSpinePairCounterexample
expectLengthSpinePairPositiveAffineApplicableDomainCounterexample problem
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected positive-affine product-domain rejection: " ++ show failure)
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the violating positive-affine product domain was inapplicable: " ++
      show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "the violating positive-affine product domain produced positive evidence"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthSpinePairProblemBehavioralProblem problem)
...
assertValidatedLengthPositiveAffineApplicableDomain
  ...
  -> Evaluate.LengthCounterexampleBasis
  -> IO ()
assertValidatedLengthPositiveAffineApplicableDomain receipt maximums
    assignments applicable basis = do
  Evaluate.validatedLengthPositiveAffineApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthPositiveAffineApplicableDomainAssignmentCount
      receipt @?= assignments
  Evaluate.validatedLengthPositiveAffineApplicableDomainApplicableAssignmentCount
      receipt @?= applicable
  Evaluate.validatedLengthPositiveAffineApplicableDomainBasis receipt @?= basis
...
assertValidatedLengthSpinePairPositiveAffineApplicableDomain
  ...
  -> Evaluate.LengthCounterexampleBasis
  -> IO ()
assertValidatedLengthSpinePairPositiveAffineApplicableDomain receipt maximums
    assignments applicable basis = do
  Evaluate.validatedLengthSpinePairPositiveAffineApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthSpinePairPositiveAffineApplicableDomainAssignmentCount
      receipt @?= assignments
  Evaluate.validatedLengthSpinePairPositiveAffineApplicableDomainApplicableAssignmentCount
      receipt @?= applicable
  Evaluate.validatedLengthSpinePairPositiveAffineApplicableDomainBasis receipt
    @?= basis

type AdversarialLengthRelationalPositiveAffineApplicableDomainValidation =
  Either Evaluate.LengthApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteListSpineLengthV1
      ...
      (SemanticProblem.BehavioralEvidence
        Length.FiniteListSpineLengthV1
        Evaluate.ValidatedLengthRelationalPositiveAffineApplicableDomain))

type AdversarialLengthSpinePairRelationalPositiveAffineApplicableDomainValidation =
  Either Evaluate.LengthSpinePairApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
      ...
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
        Evaluate.ValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain))

expectLengthRelationalPositiveAffineApplicableDomainInapplicability
  :: AdversarialLengthRelationalPositiveAffineApplicableDomainValidation
  -> IO Evaluate.LengthApplicableDomainInapplicability
expectLengthRelationalPositiveAffineApplicableDomainInapplicability
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected relational-positive-affine domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> pure reason
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "an inapplicable relational-positive-affine domain produced a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "an inapplicable relational-positive-affine domain produced positive evidence"
      >> error "unreachable"
...
expectValidatedLengthRelationalPositiveAffineApplicableDomain
  :: LengthProblem.CheckedLengthProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthRelationalPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthRelationalPositiveAffineApplicableDomain
expectValidatedLengthRelationalPositiveAffineApplicableDomain problem
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected relational-positive-affine domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the relational-positive-affine domain was not bounded: " ++ show reason)
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the relational-positive-affine domain unexpectedly contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence
...
expectLengthRelationalPositiveAffineApplicableDomainCounterexample
  ...
  -> AdversarialLengthRelationalPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthCounterexample
expectLengthRelationalPositiveAffineApplicableDomainCounterexample problem
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected relational-positive-affine domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the violating relational-positive-affine domain was inapplicable: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "the violating relational-positive-affine domain produced positive evidence"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample evidence) ->
    expectRight $ SemanticProblem.replayBehavioralEvidence
      (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence
...
expectValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain
  :: LengthProblem.CheckedLengthSpinePairProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthSpinePairRelationalPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain
expectValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain problem
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected relational-positive-affine product-domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the relational-positive-affine product domain was not bounded: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the relational-positive-affine product domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthSpinePairProblemBehavioralProblem problem)
...
expectLengthSpinePairRelationalPositiveAffineApplicableDomainCounterexample
  ...
  Left failure -> assertFailure
    ("unexpected relational-positive-affine product-domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the violating relational-positive-affine product domain was inapplicable: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "the violating relational-positive-affine product domain produced positive evidence"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample evidence) ->
    expectRight $ SemanticProblem.replayBehavioralEvidence
      (LengthProblem.checkedLengthSpinePairProblemBehavioralProblem problem)
...
assertValidatedLengthRelationalPositiveAffineApplicableDomain
  ...
  -> Evaluate.LengthCounterexampleBasis
  -> IO ()
assertValidatedLengthRelationalPositiveAffineApplicableDomain receipt maximums
    assignments applicable basis = do
  Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainAssignmentCount
  ...
  Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainApplicableAssignmentCount
      receipt @?= applicable
  Evaluate.validatedLengthRelationalPositiveAffineApplicableDomainBasis receipt
    @?= basis
...
assertValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain
  ...
  -> Evaluate.LengthCounterexampleBasis
  -> IO ()
assertValidatedLengthSpinePairRelationalPositiveAffineApplicableDomain receipt
    maximums assignments applicable basis = do
  Evaluate.validatedLengthSpinePairRelationalPositiveAffineApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthSpinePairRelationalPositiveAffineApplicableDomainAssignmentCount
      receipt @?= assignments
  Evaluate.validatedLengthSpinePairRelationalPositiveAffineApplicableDomainApplicableAssignmentCount
      receipt @?= applicable
  Evaluate.validatedLengthSpinePairRelationalPositiveAffineApplicableDomainBasis
      receipt @?= basis

type AdversarialLengthStrictRelationalPositiveAffineApplicableDomainValidation =
  Either Evaluate.LengthApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteListSpineLengthV1
...
type AdversarialLengthSpinePairStrictRelationalPositiveAffineApplicableDomainValidation =
  Either Evaluate.LengthSpinePairApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
      ...
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
        Evaluate.ValidatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomain))

expectLengthStrictRelationalPositiveAffineApplicableDomainInapplicability
  :: AdversarialLengthStrictRelationalPositiveAffineApplicableDomainValidation
  -> IO Evaluate.LengthApplicableDomainInapplicability
expectLengthStrictRelationalPositiveAffineApplicableDomainInapplicability
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> pure reason
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "an inapplicable strict-relational-positive-affine domain produced a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "an inapplicable strict-relational-positive-affine domain produced positive evidence"
      >> error "unreachable"
...
expectValidatedLengthStrictRelationalPositiveAffineApplicableDomain
  :: LengthProblem.CheckedLengthProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthStrictRelationalPositiveAffineApplicableDomainValidation
  -> IO Evaluate.ValidatedLengthStrictRelationalPositiveAffineApplicableDomain
expectValidatedLengthStrictRelationalPositiveAffineApplicableDomain problem
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the strict-relational-positive-affine domain was not bounded: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the strict-relational-positive-affine domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence
...
expectValidatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomain
  :: LengthProblem.CheckedLengthSpinePairProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthSpinePairStrictRelationalPositiveAffineApplicableDomainValidation
  -> IO
      Evaluate.ValidatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomain
expectValidatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomain
    problem validation = case validation of
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine product-domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the strict-relational-positive-affine product domain was not bounded: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the strict-relational-positive-affine product domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthSpinePairProblemBehavioralProblem problem)
...
assertValidatedLengthStrictRelationalPositiveAffineApplicableDomain
  ...
  -> Evaluate.LengthCounterexampleBasis
  -> IO ()
assertValidatedLengthStrictRelationalPositiveAffineApplicableDomain receipt
    maximums assignments applicable basis = do
  Evaluate.validatedLengthStrictRelationalPositiveAffineApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthStrictRelationalPositiveAffineApplicableDomainAssignmentCount
...
assertValidatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomain
  ...
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineApplicableDomainAssignmentCount
      receipt @?= assignments
...
type AdversarialLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidation =
  Either Evaluate.LengthApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteListSpineLengthV1
...
type AdversarialLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidation =
  Either Evaluate.LengthSpinePairApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
      ...
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
        Evaluate.ValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain))

expectLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainInapplicability
  :: AdversarialLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidation
  -> IO Evaluate.LengthApplicableDomainInapplicability
expectLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainInapplicability
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine-quotient-root-extrema domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> pure reason
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "an inapplicable strict-relational-positive-affine-quotient-root-extrema domain produced a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "an inapplicable strict-relational-positive-affine-quotient-root-extrema domain produced positive evidence"
      >> error "unreachable"
...
expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
  ...
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine-quotient-root-extrema domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the strict-relational-positive-affine-quotient-root-extrema domain was not bounded: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the strict-relational-positive-affine-quotient-root-extrema domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence
...
expectValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
  :: LengthProblem.CheckedLengthSpinePairProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainValidation
  -> IO
      Evaluate.ValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
expectValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
    problem validation = case validation of
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine-quotient-root-extrema product-domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the strict-relational-positive-affine-quotient-root-extrema product domain was not bounded: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the strict-relational-positive-affine-quotient-root-extrema product domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthSpinePairProblemBehavioralProblem problem)
...
assertValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomain
    receipt maximums assignments applicable basis = do
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainAssignmentCount
      receipt @?= assignments
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainApplicableAssignmentCount
      receipt @?= applicable
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaApplicableDomainBasis
      receipt @?= basis

type AdversarialLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidation =
  Either Evaluate.LengthApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteListSpineLengthV1
...
type AdversarialLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidation =
  Either Evaluate.LengthSpinePairApplicableDomainValidationError
    (Evaluate.LengthApplicableDomainValidation
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
      ...
      (SemanticProblem.BehavioralEvidence
        Length.FiniteBinaryProductSpineLengthsV1
        Evaluate.ValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain))

expectLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainInapplicability
  :: AdversarialLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidation
  -> IO Evaluate.LengthApplicableDomainInapplicability
expectLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainInapplicability
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine-quotient-root-extrema-monus domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> pure reason
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "an inapplicable strict-relational-positive-affine-quotient-root-extrema-monus domain produced a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "an inapplicable strict-relational-positive-affine-quotient-root-extrema-monus domain produced positive evidence"
      >> error "unreachable"
...
expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
  ...
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine-quotient-root-extrema-monus domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the strict-relational-positive-affine-quotient-root-extrema-monus domain was not bounded: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the strict-relational-positive-affine-quotient-root-extrema-monus domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence
...
expectValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
  :: LengthProblem.CheckedLengthSpinePairProblem
      AdversarialIdentity AdversarialLocal
  -> AdversarialLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainValidation
  -> IO
      Evaluate.ValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
expectValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
    problem validation = case validation of
  Left failure -> assertFailure
    ("unexpected strict-relational-positive-affine-quotient-root-extrema-monus product-domain rejection: "
      ++ show failure) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the strict-relational-positive-affine-quotient-root-extrema-monus product domain was not bounded: "
      ++ show reason) >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the strict-relational-positive-affine-quotient-root-extrema-monus product domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthSpinePairProblemBehavioralProblem problem)
...
assertValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomain
    receipt maximums assignments applicable basis = do
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainInclusiveMaximums
      receipt @?= maximums
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainAssignmentCount
      receipt @?= assignments
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainApplicableAssignmentCount
      receipt @?= applicable
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusApplicableDomainBasis
      receipt @?= basis

type AdversarialLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainValidation =
...
expectLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainInapplicability
  :: AdversarialLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainValidation
  -> IO Evaluate.LengthApplicableDomainInapplicability
expectLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainInapplicability
    validation = case validation of
  Left failure -> assertFailure
    ("unexpected Boolean finite-union domain rejection: " ++ show failure)
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> pure reason
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "an inapplicable Boolean finite union produced a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished _) -> assertFailure
    "an inapplicable Boolean finite union produced positive evidence"
      >> error "unreachable"
...
expectValidatedLengthStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
  ...
  Left failure -> assertFailure
    ("unexpected Boolean finite-union domain rejection: " ++ show failure)
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainInapplicable reason) -> assertFailure
    ("the Boolean finite-union domain was not bounded: " ++ show reason)
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainCounterexample _) -> assertFailure
    "the Boolean finite-union domain contained a counterexample"
      >> error "unreachable"
  Right (Evaluate.LengthApplicableDomainEstablished evidence) -> expectRight
    $ SemanticProblem.replayBehavioralEvidence
        (LengthProblem.checkedLengthProblemBehavioralProblem problem) evidence
...
assertValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
  :: Evaluate.ValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
  -> [[Natural]]
  -> Natural
  -> Natural
  -> Natural
...
assertValidatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomain
    receipt boxes boxCount visits assignments applicable basis = do
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainInclusiveMaximumBoxes
      receipt @?= boxes
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainBoxCount
      receipt @?= boxCount
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainAssignmentVisitCount
      receipt @?= visits
  Evaluate.validatedLengthSpinePairStrictRelationalPositiveAffineQuotientRootExtremaMonusBooleanFiniteUnionApplicableDomainAssignmentCount
      receipt @?= assignments
